...
typedef struct {
	HelpEntry	help;   /* what to show when help is selected */
	NRowCol		loc;    /* location of item on screen */
	FieldType	type;   /* right/left justified, etc */
	char		*label; /* label to show */
	int		len;    /* length of label */
	int		maxlen; /* max length, allows for scrollable items */
	void		*data;  /* user-specified data */
	ChooseModifier	choosefieldtype; /* modified field flags */
} Choose_item_t;
...
extern int	wget_field(WINDOW *w, int, int, int, int, int, char *,
		u_long);
extern int	wget_field_noedit(WINDOW *w, int, int, int, int, int, char *,
		u_long);
...
extern char	*replace_chars(char *, char);
extern int tty_CheckInput(ulong which_fkeys, int ch);
extern char *fit_diskname_to_length(char *, int);
