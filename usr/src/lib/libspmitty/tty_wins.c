...
#pragma ident	"@(#)tty_wins.c	1.14	07/10/09 SMI"
...
static	int	_wget_field(WINDOW *, int, int, int, int, int, char *, u_long,
				int);
...
/*
 * Name:	wget_field_nonechoed
 * Description:	Begins routine to accept a line of input on a curses field.
 *		As text is typed into the field, a replacement "Echo" character
 *		is shown.  Useful for password fields where the passsword should
 *		not be shown in cleartext.
 * Scope:	public
 * Arguments:	w - The curses window
 *		type - The type of field.  See spmitty_api.h
 *		x - The x position of the first character of the field
 *		y - The y position of the first character of the field
 *		len - The length of the field onscreen
 *		max - The max # characters the field can hold
 *		val - The initial value to display
 *		fkeys - which fkeys are valid
 *		echochar - The character to show in place of typed characters.
 * Returns:	The control character typed.  Note that normal
 *		characters do not cause a return.
 */
int
wget_field_nonechoed(WINDOW * w,
    int type,
    int x,
    int y,
    int len,
    ...
    char echochar)
{
	...
	if (echochar == '\0') {
		return (wget_field(w, type, x, y, len, max, val, fkeys));
	}
	...
	(void) strcpy(buf, replace_chars(val, echochar));
	(void) strcpy(osbuf, val);

	if ((int)strlen(buf) > len) {
		scrolled = 1;
		cp1 = buf + (strlen(buf) - len);	/* cp1 == len chars */
		oscp1 = osbuf + (strlen(osbuf) - len);	/* cp1 == len chars */
	} else {
		cp1 = buf;
		oscp1 = buf;
	}
	cp = buf + strlen(buf);
	oscp = osbuf + strlen(osbuf);
...
wget_field(WINDOW * w,
    ...
    u_long fkeys)
{
	return (_wget_field(w, type, x, y, len, max, val, fkeys, 1));
}

int
wget_field_noedit(WINDOW * w,
    int type,
    int x,
    int y,
    int len,
    ...
    u_long fkeys)
{
	...
	return (_wget_field(w, type, x, y, len, max, val, fkeys, 0));
}

int
_wget_field(WINDOW * w,
    int type,
    int x,
    int y,
    int len,
    ...
    int editable)
{
	...
	if ((int)strlen(buf) > len) {
		scrolled = 1;
		/*
		 * Add 1 to account for scroll char, '<'
		 */
		cp1 = buf + (strlen(buf) - len) + 1;	/* cp1 == len chars */
	} else
		cp1 = buf;
	...
	for (;;) {
		...
		if (ch == erasech || ch == '\010') {
			...
			if (scrolled) {
				cp1--;
				/*
				 * If transitioning from scroll to no scroll,
				 * need to account for scroll char
				 */
				if ((cp1 - 1) == buf) {
					scrolled = 0;
					cp1--;
			...
			if ((cp - buf) < len) {
				...
				*(cp++) = (char)ch;
				cp1++;
				/*
				 * If transitioning from no scroll to scroll,
				 * need to account for scroll char
				 */
				if (scrolled == 0) {
					cp1++;
