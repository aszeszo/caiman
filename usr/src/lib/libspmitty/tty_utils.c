...
char
*replace_chars(char *str, char replacechar)
{
	...
	if (rstring == NULL) {
		rstring = (char *)xmalloc((strlen(str) + 1) * sizeof (char));
	} else {
	    rstring = xrealloc(rstring, (strlen(str) + 1) * sizeof (char));
	}
	memset(rstring, '\0', strlen(str) + 1);
	memset(rstring, replacechar, strlen(str));
	return (rstring);
}
...
char *
fit_diskname_to_length(char *diskname, int length)
{

	char *ptr;
	char *ret_str;
	char first_five_char[5];

	if (diskname == NULL) {
		return(NULL);
	}

	if (strlen(diskname) <= length) {
		return(xstrdup(diskname));
	}

	/*
	 * If the diskname is too long, the first 5 characters followed by
	 * "...' and last (length-7) characters
	 * For example if the disk name is ABCDEFGHIJKLMNOPQRSTUVWXYZ and 
	 * the length is 15, you get the display string
	 * ABCDE...TUVWXYZ
	 */
	(void) strncpy(first_five_char, diskname, sizeof(first_five_char));
	ptr = diskname + strlen(diskname) - length + 8 ;	

	ret_str = (char *) xmalloc(length+1);
	(void) snprintf(ret_str, length+1, "%s...%s", first_five_char, ptr);
	return(ret_str);
}
