...
#pragma ident	"@(#)srchcfile.c	1.21	07/11/06 SMI"
...
static int	getnum(char **cp, int base, long *d, long bad);
...
narrowSearch(VFP_T *a_vfp, char *a_path, size_t a_pathLen)
{
	...
	while (ISPKGPATHSEP[(int)((unsigned char *)*pmid)] == 0) {
		pmid++;
	}
...
srchcfile(struct cfent *ept, char *path, VFP_T *cfVfp, VFP_T *cfTmpVfp)
{
	...
	for (;;) {
		...
		if (c != '/') {
			...
			case 'x':
				...
				while (ISPKGPATHSEP[(int)((unsigned char *)*p)]
									== 0) {
					p++;
				}
		...
		/* copy first token into path element of passed structure */
			...
			while (ISPKGPATHSEP[(int)((unsigned char *)*p)] == 0) {
				p++;
			}
	...
	if (((ept->ftype == 'c') || (ept->ftype == 'b'))) {
		...
		if (getnum(&vfpGetCurrCharPtr(cfVfp), 10,
				(long *)&ept->ainfo.major, BADMAJOR) ||
		    getnum(&vfpGetCurrCharPtr(cfVfp), 10,
	...
	if ((ept->ftype == 'd') || (ept->ftype == 'x') || (ept->ftype == 'c') ||
		...
		if (getnum(&vfpGetCurrCharPtr(cfVfp), 8,
...
static int
getnum(char **cp, int base, long *d, long bad)
{
	int c;
	char	*p = *cp;
