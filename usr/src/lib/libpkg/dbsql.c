...
#pragma ident	"@(#)dbsql.c	1.13	06/02/27 SMI"
...
convert_db_to_pkginfo(const char *pcPkg, genericdb *pdb, const char *pcroot)
{
	...
	(void) sprintf(path, "%s/var/sadm/pkg/%s/pkginfo", pcroot, pcPkg);
	...
	for (k = 0; k < nrd; k++) {
		...
		if (genericdb_result_table_str(r2, k,
			...
			1, &pc2) != genericdb_OK) {
			DEBUG("get parameter & value", NULL);
			result = 1;
			goto to_pkginfo_done;
		}
...
convert_db_to_depend(const char *pcPkg, genericdb *pdb, const char *pcroot)
{
	...
	(void) sprintf(path, "%s/var/sadm/pkg/%s/install/depend",
		pcroot, pcPkg);
