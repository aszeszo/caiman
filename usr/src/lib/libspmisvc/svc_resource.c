...
#pragma ident	"@(#)svc_resource.c	1.38	07/10/09 SMI"
 ...
 * Description:	Calculate the swap space required.
 *
 *		The default is always 512.
 *
...
coreResobjContentCache(ResobjHandle res, ResSize_t flag)
{
	...
	if (flag == RESSIZE_DEFAULT) {
		...
		if (dp != NULL) {
			if (sdisk_is_usable(dp)) {
				avail = sdisk_max_hole_size(dp, GEOM_IGNORE,
				    GEOM_IGNORE);
				if (avail >= min)
					return (avail);
			}
 ...
 *			  physical		 swap
 *			  --------		 ----
 *			  0 - 128  MB	     64-192 MB (see below)
 *			128 - 512  MB		128 MB
 *			   > 512   MB		256 MB
 *
 *		Machines with 0-128MB of RAM will get enough swap such that
 *		the total virtual memory is at least 192MB.  The default
 *		swap size will be rounded up to the next 64MB.
 *
