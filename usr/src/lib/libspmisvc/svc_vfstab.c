...
void		_create_x86boot_entry(Vfsent **vlist);
int		_mount_add_nonufs_entry(Vfsent **, char *, char *,
			char *, char *, int, char *);
...
_create_mount_list(Disk_t *dlist, Dfs *rlist, Vfsent **vlist)
{
	...
	/*
	 * add the x86boot partition
	 */
	_create_x86boot_entry(vlist);
...
/*
 * Function:	_create_x86boot_entry
 * Description:	Create a vfstab entry for the x86boot partition (if necessary)
 * Scope:	internal
 * Parameters:	Vfsent **  - pointer to head of vfsent linked list
 * Return:	none
 */
void
_create_x86boot_entry(Vfsent **vlist)
{
	...
	if (BootobjGetAttribute(CFG_CURRENT,
				...
				BOOTOBJ_STUBBOOT_PARTNO, &stubpartno,
				NULL) != D_OK) {
		/* We really don't have a way to deal with a failure */
		return;
	}

	/* Nothing to do if there's no x86boot partition */
	if (stubdisk[0] == '\0') {
		return;
	}

	/* Build the device path */
	sprintf(devpath, "/dev/dsk/%sp0:boot", stubdisk);

	_mount_add_nonufs_entry(vlist, MNTTYPE_PCFS, BOOT, devpath, NULL,
				FALSE, NULL);
}
...
_mount_add_local_entry(Vfsent **vlist, Disk_t *dp, int slice)
{
	...
	/*
	 * If the slice configured to be a mirror device, mount the mirror
	 * volume
	 */
	tmp = get_mirror_block_device(disk_name(dp), slice);
	if (tmp) {
		vp->vfs_special = xstrdup(tmp);
	} else {
		vp->vfs_special = xstrdup(
			make_block_device(disk_name(dp), slice));
	}
	...
	if (slice_is_swap(dp, slice)) {
		...
		tmp = get_mirror_char_device(disk_name(dp), slice);
		if (tmp) {
			vp->vfs_fsckdev = xstrdup(tmp);
		} else {
...
/*
 * Function:	_mount_add_nonufs_entry
 * Description:	Create a mount list structure for a non-ufs filesystem and add
 *		it to the end of the 'vlist' linked list.
 *
 * Scope:	internal
 * Parameters:	vlist	- address of pointer to head of mount point list
 *		fstype	- the type of the filesystem on the device
 *		mountp	- mount point for device
 *		devname	- block device to be mounted
 *		fsckname - raw device name for fscking (NULL if none)
 *		automnt - int to set automount to yes or no
 *		mntopts	- mount options to be used (NULL if none)
 * Return:	ERROR	- failure to allocate mount list structure
 *		NOERR	- add succeeded
 */
int
_mount_add_nonufs_entry(Vfsent **vlist, char *fstype, char *mountp,
			char *devname, char *fsckname, int automnt,
			char *mntopts)
{
	struct vfstab	*vp;

	/* validate parameters */
	if (vlist == NULL || devname == NULL || fstype == NULL)
		return (ERROR);
	...
	vp->vfs_special = xstrdup(devname);
	if (fsckname != NULL) {
		vp->vfs_fsckdev = xstrdup(fsckname);
		vp->vfs_fsckpass = (_filesys_boot_critical(vp->vfs_mountp)) ?
					"1" : "3";
	}
	vp->vfs_mountp = xstrdup(mountp);
	vp->vfs_fstype = xstrdup(fstype);
	if (mntopts != NULL) {
		vp->vfs_mntopts = xstrdup(mntopts);
	}

	vp->vfs_automnt = xstrdup((automnt == TRUE) ? "yes" : "no");
 ...
 * Parameters:	all	- whether or not to mount all filesystems, regardless
 *			of if they may be being fsck'd or newfs'd in the
 *			background.
 * Return:	NOERR	- no error
 *		ERROR	- error occurred
 */
int
_mount_filesys_all(OpType Operation, Vfsent *vlist, int all)
{
	Vfsent		*vp;
	char		buf[MAXPATHLEN];
	struct vfstab	*vfsp;
	...
	WALK_LIST(vp, vlist) {
		 ...
		 * out all file systems with types other than ufs, s5, or pcfs
		 */
		...
		if (Operation == SI_ADAPTIVE) {
			...
			/*
			 * If the file system is not a ufs, System V,
			 * or pcfs file system then we don't try to mount it.
			 */

			else if (!streq(vfsp->vfs_fstype, "ufs") &&
				!streq(vfsp->vfs_fstype, "s5") &&
				!streq(vfsp->vfs_fstype, "pcfs")) {
				continue;
			}
		}
		 ...
		 * If 'async' is true, this means we should check to see
		 * if filesystems might potentially be newfs'ing or fsck'ing
		 * in the background.  If they are, we don't mount them.
		 */
		if (!all && _mount_synchronous_fs(vfsp, vlist) == 0)
			continue;
...
_mount_filesys_specific(char *name, struct vfstab *vp)
{
	...
	if (!GetSimulation(SIM_EXECUTE)) {
		...
		if (streq(vp->vfs_fstype, MNTTYPE_UFS))
...
_merge_mount_list(OpType Operation, Vfsent **vlist)
{
	...
	char		vfile[32] = "";
	char		*v = NULL;
	char		install_devlink[MAXPATHLEN];
	...
	if ((fp = fopen(vfile, "r")) == NULL) {
		...
		for (vfsnull(&vfstab); getvfsent(fp, &vfstab) == 0;
			 ...
			 */
			if ((_map_to_effective_dev(vfstab.vfs_special,
			    install_devlink) == 0) &&
			    ((dp = find_disk(install_devlink)) != NULL) &&
			    disk_selected(dp)) {
				continue;
			}
			 ...
			 * function contains all of the mount points required
			 * to mount the new file system layout.
			 */

			if (Operation == SI_ADAPTIVE ||
			    Operation == SI_FLASH_INSTALL) {
				...
				if (Operation == SI_FLASH_INSTALL) {
					/*
					 * In the flash case, we don't
					 * care if the filesystem is a
					 * real filesystem on this system
					 * or not, it should be removed as
					 * it was generated on a different
					 * (master) system than the one
					 * being currently installed
					 * (clone), and is most likely bogus
					 * anyways.
					 */
			...
			if ((streq(vfstab.vfs_fstype, "swap") ||
			    ...
			    (_map_to_effective_dev(vfstab.vfs_special,
				install_devlink) == 0)) {
				vp->vfs_special = xstrdup(install_devlink);
			} else {
			...
			if ((streq(vfstab.vfs_fstype, "swap") ||
			    ...
			    (_map_to_effective_dev(vfstab.vfs_fsckdev,
				install_devlink) == 0)) {
				vp->vfs_fsckdev = xstrdup(install_devlink);
			} else {
...
_mount_synchronous_fs(struct vfstab *vfsp, Vfsent *vlist)
{
	...
	/*
	 * filesystems that are neither UFS nor pcfs are not processed
	 * synchronously
	 */
	if (!streq(vfsp->vfs_fstype, MNTTYPE_UFS) &&
	    !streq(vfsp->vfs_fstype, MNTTYPE_PCFS))
		return (0);

	/* this should never happen because of the FS type test, but... */
	if (vfsp->vfs_mountp == NULL)
		return (0);

	/* if the entry is a system ancestor, it is synchronous */
