...
#pragma ident	"@(#)svc_services.c	1.11	07/10/09 SMI"
...
int validate_service_modification(SW_service_modspec *, SW_error_info **);
int execute_service_modification(SW_service_modspec *, SW_error_info **);
		...
		*karch++ = '\0';
		...
		for (; modp; modp = modp->next) {
			if (modp->type != METACLUSTER)
				break;
			meta = (SW_metacluster *)xcalloc((size_t)
			    sizeof (SW_metacluster));
...
list_installed_svc_platforms(SW_service *svc, SW_error_info **err_info)
{
	...
	(void) sprintf(service_name, "%s_%s", svc->sw_svc_os,
	    svc->sw_svc_version);
	...
	SW_error_info **err_info)
{
	 ...
	 * And it is used to create a seperate usr area for
	 * diskless clients.
	 */
	...
	SW_error_info **err_info)
{
	...
	SW_service *new_service = modsp->sw_svmodspec_mods->sw_svmod_newservice;
	...
	(void) sprintf(log, "/var/sadm/system/logs/%s_%s_%s_%s",
		new_service -> sw_svc_os,
		new_service -> sw_svc_version,
		new_service -> sw_svc_isa,
		new_service -> sw_svc_plat);
	...
	if (realstat != 0) {
		*err_info = (SW_error_info *)xcalloc((size_t)
		    sizeof (SW_error_info));
...
get_createroot_info(SW_service *svc, SW_error_info **err_info)
{
	...
	cri = (SW_createroot_info *)xcalloc((size_t)
		sizeof (SW_createroot_info));
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (streq(key, "USR_PATH")) {
			...
			remmntp = (SW_remmnt *)xcalloc((size_t)
			    sizeof (SW_createroot_info));
			...
			pkgp = (SW_pkgadd_def *)xcalloc((size_t)
			    sizeof (SW_pkgadd_def));
...
do_add_service(SW_service_mod *msp, SW_error_info **err_info)
{
	...
	for (lp = msp->sw_svmod_locales; lp; lp = lp->next) {
		stat = select_locale(prodmod, lp->sw_loc_name, FALSE);
	}
	return (0);
}
    ...
    SW_error_info **err_info)
{
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (streq(key, "SPOOLED_ROOT") || streq(key, "KVM_PATH") ||
			...
			/* save the full arch string */
			(void) strcpy(fullarch, isa);
...
is_pkg(char *spooled_root)
{
	...
	(void) sprintf(pkginfo_cmd,
	    "/usr/bin/pkginfo  -d %s >> /dev/null 2>&1", spooled_root);
