...
#pragma ident	"@(#)svc_sdiskconfig.c	1.41	07/10/09 SMI"
...
SdiskobjAutolayout(void)
{
	...
	if (get_trace_level() > 0) {
		write_debug(SVC_DEBUG_L1(1),
			"Entering Autolayout: resource list");
		ResobjPrintList();
	}
	...
	if (AdvanceBootobj(TRUE) != D_OK) {
		write_debug(SVC_DEBUG_L1(1),
			"Autolayout: AdvanceBootobj failed");
		return (D_FAILED);
	}
...
SliceobjConfigResobj(ResobjHandle res, Disk_t *dp)
{
	...
	/* obey and prom-specific limitations for root */
	if (streq(Resobj_Name(res), ROOT)) {
		end = disk_geom_lcyl(dp);
		disk_geom_lcyl(dp) = disk_geom_lrootcyl(dp);
	}

	if (size > sdisk_max_hole_size(dp, GEOM_IGNORE, GEOM_IGNORE)) {
		if (streq(Resobj_Name(res), ROOT)) {
			/* reset prom-specific limitation */
			disk_geom_lcyl(dp) = end;
		}
		return (D_NOSPACE);
	}
...
SyncConfig(void)
{
	...
	write_debug(SVC_DEBUG_L1(5), "SyncConfig: walking resources");
	WALK_RESOURCE_LIST(res, RESTYPE_UNDEFINED) {
		write_debug(SVC_DEBUG_L1_NOHD(5),
			"resource: %s", Resobj_Name(res));
		if ((key = SliceobjFindUse(CFG_CURRENT, NULL, Resobj_Name(res),
				Resobj_Instance(res), TRUE)) != NULL) {

			write_debug(SVC_DEBUG_L1_NOHD(5),
				"Key (disk/slice): %s/%d",
				disk_name(key->dp), key->slice);
...
InitBootobjDisk(void)
{
	...
	if ((res = ResobjFind(ROOT, 0)) != NULL &&
			ResobjGetAttribute(res,
				...
				NULL) == D_OK &&
			disk[0] != '\0' && disk_selected(find_disk(disk))) {
