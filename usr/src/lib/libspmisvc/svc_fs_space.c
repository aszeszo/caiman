...
#pragma ident	"@(#)svc_fs_space.c	1.21	07/10/09 SMI"
...
static int	calc_devfs_filesize(const char *, const struct stat *, int,
			struct FTW *);
static void	add_archive_space(FSspace **);
 ...
 * Parameters:
 * Return:
 *	SUCCESS			- The proposed layout is sufficient
 *	SP_ERR_NOT_ENOUGH_SPACE	- The proposed layout is too small
 *	FAILURE			- Unable to verify proposed layout
 * Status:
 *	public
 */
int
    ...
    void *callback_arg)
{
	...
	if (calc_sw_fs_usage(sp, callback_proc, callback_arg) != SUCCESS)
		return (FAILURE);
	...
	if (is_upgrade()) {
		...
		for (i = 0; sp[i]; i++) {
			...
			if (sp[i]->fsp_fsi) {
				...
				device = sp[i]->fsp_fsi->fsi_device;
				/*
				 * check to see if the
				 * device is a metadevice
				 */
				if (isMeta(device) == TRUE) {
					/*
					 * it is a metadevice, get
					 * the smallest component
					 */
					device = getSmallestMetaComp(device);
				}
...
space_meter(char **mplist)
{
	...
	static	boolean_t prev_null = B_FALSE;
	int	i, j;
	FSspace	**mtab;
	...
	/*
	 * Don't do normal media space checking for flash installs
	 */
	if (!is_flash_install()) {

		if ((mod = get_media_head()) == (Module *)NULL) {
#ifdef DEBUG
			(void) printf("DEBUG: space_meter(): " \
			    "media head NULL\n");
#endif
			return (NULL);
		}
	...
	/*
	 * add overhead to '/' to account for x86 boot archives
	 */
	add_archive_space(new_sp);
...
/*
 * Function:	add_archive_space
 * Description: Add size for boot_archive and failsafe archive
 *		if we are upgrading from realmode boot x86
 *		system to grub boot.
 * Scope:	private
...
add_archive_space(FSspace **sp)
{
	...
	if (sp[i] != NULL) {
		/*
		 * The boot archive is a compressed image of kernel
		 * modules. There is no cheap way to calculate the
		 * size, short of constructing it.
		 * The normal boot archive adds 22 MB and the failsafe
		 * archive adds another 46 MB. To leave some room,
		 * we reserve 80 MB.
		 */
		extra = mb_to_kb(80);

		/* add the resulting space to the '/' space table entry */
		fsp_add_to_field(sp[i], FSP_CONTENTS_NONPKG, extra);
	}
}
		...
		struct FTW *ftwp)
{
	 ...
	 *
	 * `suninstall' is a lamb, led astray by the wolf known as stat (to
	 * paraphrase dmick@west).  We were trying to get the sizes of all
	 * directories, symlinks, and non-character/block-special devices.
	 * We thought we could do this by sizing (in addition to directories
	 * and symlinks) all files whose block count as reported by stat()
	 * was >0.  In some cases, a stat() on a c/b node will return the
	 * size of the disk (or the biggest thing that can fit in an off_t).
	 * As such, we now filter out all FTW_F's that are character or
	 * block specials.
	 */
	if (ftype == FTW_D || ftype == FTW_SL ||
	    (ftype == FTW_F && !(statp->st_mode & (S_IFCHR|S_IFBLK)) &&
	    statp->st_blocks > 0)) {
...
add_upg_fs_overhead(FSspace **sp)
{
	...
	for (i = 0; sp[i]; i++) {
		...
		extra = (ulong)(sp[i]->fsp_reqd_contents_space *
		    ((float)oh / 100));
		fsp_add_to_field(sp[i], FSP_CONTENTS_REQD_FREE, extra);

		if (sp[i]->fsp_cts.contents_inodes_used == 0)
			continue;

		sp[i]->fsp_cts.contents_inodes_used +=
		    (ulong)(sp[i]->fsp_cts.contents_inodes_used *
		    ((float)oh) / 100);
	}
}
...
roundup(float f)
{
	...
	/* LINTED [truncation here is desired] */
	u = f;
