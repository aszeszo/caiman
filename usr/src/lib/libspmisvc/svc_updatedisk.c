...
#pragma ident	"@(#)svc_updatedisk.c	1.25	07/10/09 SMI"
...
static int 	_create_ufs(Disk_t *dp, int, int);
...
static int	_create_filesystems(Disk_t *dlist, Vfsent *vlist, int);
static int	_create_ufs_filesystems(Disk_t *dlist, Vfsent *vlist, int);
 ...
 *		nodiskops - if true, don't do critical disk setup like
 *			VTOC writing.
 *		just make the filesystems.
 *		waitall	- whether to create some filesystems asynchronously,
 *			or all synchronously.
 * Return:	NOERR	- successful updating of disk state
 *		ERROR	- update failed
 */
int
_setup_disks(Disk_t *dlist, Vfsent *vlist, int nodiskops, int waitall)
{
	/*
	 * update the fdisk and Solaris VTOC labels on all selected drives
	 */
	if ((!nodiskops) && (_label_disks(dlist) == 0)) {
	 ...
	 *
	 * Only start swapping if we have full control of disks (!nodiskops)
	 */
	if (!nodiskops) {
		_swap_add(dlist);
	}
	 ...
	 */
	if (_create_filesystems(dlist, vlist, waitall) != 0) {
...
_swap_add(Disk_t *dlist)
{
	...
	for (dp = dlist; dp; dp = next_disk(dp)) {
		...
		WALK_SLICES(i) {
			...
			mntdev = make_block_device(disk_name(dp), i);
			if ((exempt_swapdisk == NULL) ||
			    (strcmp(exempt_swapdisk, mntdev) != 0)) {
				(void) sprintf(cmd,
 ...
 *		All other filesystems are checked in parallel in the
 *		background.
...
_label_fdisk(Disk_t *dp)
{
	...
	int	c;
	int	active;
	Disk_t	*bdp;		/* Boot disk object */
	Disk_t	*sbdp;		/* Stub boot disk object */
	int	sbpn;		/* Stub boot fdisk partition number */
	boolean_t	solaris2_found = B_FALSE;
	int		part, solpart;
	...
	/*
	 * Get the boot disk/partition information so we know what to mark
	 * active.
	 */
	(void) DiskobjFindBoot(CFG_CURRENT, &bdp);
	(void) DiskobjFindStubBoot(CFG_CURRENT, &sbdp, &sbpn);
	...
	/*
	 * Check whether the new solaris partition (SUNIXOS2) is defined already
	 */
	WALK_PARTITIONS(p) {
		if (part_id(dp, p) == SUNIXOS2) {
			solaris2_found = B_TRUE;
		}
	}
	...
	WALK_PARTITIONS(c) {
		WALK_PARTITIONS(p) {
			...
			/*
			 * Determine whether or not this partition should
			 * be marked active.
			 */
			if (sbdp != NULL) {
				/*
				 * There is a stub boot partition, so mark
				 * it active.
				 */
				if (sbdp == dp) {
					if (p == sbpn) {
				...
				/*
				 * There isn't a stub boot partition, so
				 * mark the Solaris partition on the boot
				 * disk active.
				 */
				if (bdp == dp) {
					if (solaris2_found) {
...
_label_sdisk(Disk_t *dp)
{
	...
	if (!disk_is_vbd(dp) && write_alts == 1) {
...
/*
 * Function:	_create_filesystems
 * Description:	Create the various filesystems requested on the disk list.
 * Scope:	private
 * Parameters:	dlist	- pointer to list of disks
 *		vlist	- pointer to list of mount points
 *		waitall	- whether to wait for non-system filesystems
 *			to be created or not
 * Return:	 0	- successful creation of filesystems
 *		-1	- filesystem creation failed
 *		 1	- filesystem verification failed
 */
static int
_create_filesystems(Disk_t *dlist, Vfsent *vlist, int waitall)
{
	/*
	 * Create UFS filesystems
	 */
	return (_create_ufs_filesystems(dlist, vlist, waitall));
}

/*
 * Function:	_create_ufs_filesystems
 * Description: Process the entire disk list. Newfs all UFS file systems
 ...
 * Parameters:	dlist	- pointer to list of disks
 *		vlist	- pointer to list of mount points
 *		waitall	- whether to wait for all or some filesystems to
 *			be created
 ...
 */
static int
_create_ufs_filesystems(Disk_t *dlist, Vfsent *vlist, int waitall)
{
	Disk_t	*dp;
	int	i;
	int	wait;
	...
	WALK_LIST(dp, dlist) {
		...
		WALK_SLICES(i) {
			...
			/*
			 * The second submirror will not have a mount point
			 * but a filesystem has to be created
			 */
			if (slice_mntpnt_isnt_fs(dp, i)) {
				if (is_slice_tobe_mirrored(dp->name, i)) {
					if (waitall) {
						wait = 1;
					}
					if (_create_ufs(dp, i, wait) != 0) {
						return (-1);
					}
				}
				continue;
