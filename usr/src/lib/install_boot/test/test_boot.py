...
class SystemBootMenuTestCase(BootMenuTestCaseBase):
    ...
    def test_parse_doc_target(self):
        ...
        for arch in ['sparc', 'i386']:
            ...
            if self.arch == 'i386':
                iit = "Parse Doc Target Unit Test"
                # write out the GRUB_TITLE line to .image_info file on X86
                img_path = tempfile.mktemp(dir=TMP_TEST_DIR,
                                           prefix="img_info_")
                self.boot_menu.img_info_path = img_path
                with open(self.boot_menu.img_info_path, "a+") as iip:
                    iip.write("GRUB_DO_SAFE_DEFAULT=true\n")
                    iip.write("GRUB_TITLE=" + iit + "\n")
            ...
            # Verify that the .image_info GRUB_TITLE key/val matches
            # img_info_title and boot_title properties
            if self.arch == 'i386':
                self.failUnlessEqual(self.boot_menu.img_info_title, iit,
                    "Unexpected img_info_title value.\n" \
                    "Expected: '%s'\nActual: '%s'" \
                    % (iit, self.boot_menu.img_info_title))
                self.failUnlessEqual(self.boot_menu.boot_title, iit,
                    "Unexpected boot_title value.\n" \
                    "Expected: '%s'\nActual: '%s'" \
