...
#pragma ident	"@(#)xm_adminhelp.c	1.5	07/11/12 SMI"
...
create_widgets(Widget parent, char help_category, char *textfile)
{
	...
	/* set process traversal to the set radio button */
	switch ((int) help_category) {
	case TOPIC:
	default:
		XmProcessTraversal(radio[TOP], XmTRAVERSE_CURRENT);
		break;
	case HOWTO:
		XmProcessTraversal(radio[HOW], XmTRAVERSE_CURRENT);
		break;
	case REFER:
		XmProcessTraversal(radio[REF], XmTRAVERSE_CURRENT);
		break;
	}
