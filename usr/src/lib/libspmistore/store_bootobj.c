...
#pragma ident	"@(#)store_bootobj.c	1.18	07/10/09 SMI"
...
typedef struct {
	char	*disk;		/* Boot disk name (sans slice number) */
	int	device;		/* slice number (SPARC), part number (Intel) */
	char	devtype;
	char	*stubdisk;
	int	stubpartno;
	u_int	flags;
...
{
	char	fdisk[MAXPATHLEN];
	char	sdisk[MAXPATHLEN];
	int	fdevice;
	int	sdevice;
 ...
 *			    BOOTOBJ_DISK		char *	(char[MAXPATHLEN])
 ...
 *			    BOOTOBJ_STUBBOOT_DISK	char *	(char[MAXPATHLEN])
 *			    BOOTOBJ_STUBBOOT_PARTNO	int *	(&int)
 ...
 *			Attributes and values are:
 *
 *			BOOTOBJ_DISK		 char[MAXPATHLEN] (""|NULL|<disk>)
 *			BOOTOBJ_DISK_EXPLICIT	 int (0|1)
 *			BOOTOBJ_DEVICE		 int (device index)
 *			BOOTOBJ_DEVICE_EXPLICIT  int (0|1)
 *			BOOTOBJ_PROM_UPDATE	 int (0|1)
 *			BOOTOBJ_STUBBOOT_DISK	 char[MAXPATHLEN] (""|NULL|<disk>)
 *			BOOTOBJ_STUBBOOT_PARTNO  int (partition index)
 *
...
BootobjInit(void)
{
	...
	BootDefault(&disk, &device, &stubdisk, &stubpno);
	...
	if (BootobjSetAttributePriv(CFG_EXIST,
			...
			BOOTOBJ_STUBBOOT_PARTNO, stubpno,
			NULL) != D_OK) {
 ...
 *			BOOTOBJ_STUBBOOT_DISK	 char[] (disk name)
 *			BOOTOBJ_STUBBOOT_PARTNO  int (partition index)
 *
...
BootobjstateDuplicate(BootobjState *dst, BootobjState *src)
{
	...
	if (src->stubdisk != NULL && src->stubdisk[0] != '\0')
		dst->stubdisk = xstrdup(src->stubdisk);
	else
		dst->stubdisk = NULL;
...
BootobjstateFree(BootobjState *bosp)
{
	...
	if (bosp->stubdisk != NULL) {
		(void) free(bosp->stubdisk);
		bosp->stubdisk = NULL;
	}
}
...
coreBootobjGetAttribute(Label_t state, int privilege, va_list ap)
{
	...
	while ((keyword = va_arg(ap, BootobjAttrType)) != NULL) {
		switch (keyword) {
		    ...
		    case BOOTOBJ_STUBBOOT_DISK:
			vcp = va_arg(ap, char *);
			if (vcp == NULL)
				return (D_BADARG);
			if (bop->stubdisk == NULL)
				vcp[0] = '\0';
			else
				(void) strcpy(vcp, bop->stubdisk);
			break;

		    case BOOTOBJ_STUBBOOT_PARTNO:
			vip = va_arg(ap, int *);
			if (vip == NULL)
				return (D_BADARG);
			(*vip) = bop->stubpartno;
...
coreBootobjSetAttribute(Label_t state, int privilege, va_list ap)
{
	...
	while ((keyword = va_arg(ap, BootobjAttrType)) != NULL) {
		switch (keyword) {
			...
			if (vcp == NULL || vcp[0] == '\0') {
				/*
				 * reset the partition number if the disk
				 * name is being reset.
				 */
				bop->stubpartno = 0;
			} else {
				bop->stubdisk = xstrdup(vcp);
			}
			break;

		    case BOOTOBJ_STUBBOOT_PARTNO:
			vi = va_arg(ap, int);
			if (vi != 0 && invalid_fdisk_part(vi))
				return (D_BADARG);
			bop->stubpartno = vi;
			break;
...
{
	static BootobjState	Boot_object[] = {{ NULL, -1, '\0', NULL, -1, 0 },
						 { NULL, -1, '\0', NULL, -1, 0 },
						 { NULL, -1, '\0', NULL, -1, 0 }};
