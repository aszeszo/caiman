...
/* minimum size for partitions (MB) */
#define	MIN_SOLARIS_PART_SIZE	30
#define	MIN_STUBBOOT_PART_SIZE	10

/* Minimum disk size for adding a Solaris parition */
#define	MIN_DISK_SIZE	32
...
#define	GEOM_BEGINNING	-5	/* start at beginning of disk */
...
#define	DF_USESLBA	0x4000	/* disk uses LBA (exempt from 1023 cyl limit) */
...
typedef enum {
	...
	BOOTOBJ_STUBBOOT_DISK	= 8,	/* Disk containing (Intel) boot stub
					 * partition (if any)
					 */
	BOOTOBJ_STUBBOOT_PARTNO = 9	/* Partition number for (Intel) boot
					 * stub partition (if any)
					 */
} BootobjAttrType;
 ...
 */
typedef struct {
	int	firstcyl;	/* first legal data cylinder on the disk */
	int	lcyl;		/* # of accessible cylinders on disk */
	int	lrootcyl;	/* last legal cylinder for root slice */
 ...
 */
typedef struct disk {
	char	name[MAXPATHLEN]; /* short name for disk device (c0t0d0) */
...
#define	disk_uses_lba(d)		((d)->state & DF_USESLBA)
...
#define	disk_geom_lrootcyl(d)		((d)->geom.lrootcyl)
...
#define	disk_is_vbd(d)			(disk_ctype(d) == DKC_VBD)
...
#define	mb_to_cyls(d,m)		(blocks_to_cyls((d),(mb_to_blocks((d), (m)))))
...
int		fdisk_parts_avail(Disk_t *);
int		get_solaris_part(Disk_t *, Label_t);
int		get_stubboot_part(Disk_t *dp, Label_t state);
int		get_diagnostic_part(Disk_t *dp, Label_t state);
...
int		check_need_prom_boot_chg(void);
...
int		DiskobjFindStubBoot(Label_t state, Disk_t **diskp, int *partno);
int		StubBootGetBootpath(char *sdev, int spno, char **dev, int *pno,
				    int *slcp);
...
int		sdisk_max_hole_size(Disk_t *, int, int);
...
/* store_svm.c */
int		svm_set_enabled(int);
int		spmi_check_for_svm(char *);
int		spmi_start_svm(char *, svm_info_t **, int);
int		spmi_stop_svm(char *, char *);
int		remount_svm(char *, svm_info_t *, char *);
int		remount_ctds(char *, char *);
int		sliceExistsInSvm(char *, svm_info_t *);
int		isMeta(char *);
char *		getSmallestMetaComp(char *);
void		spmi_init_lib_svm();
svm_info_t *	spmi_svm_alloc();
void		spmi_svm_free(svm_info_t *);
