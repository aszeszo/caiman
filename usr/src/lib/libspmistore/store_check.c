...
#pragma ident	"@(#)store_check.c	1.17	07/10/09 SMI"
...
/*
 * Function:	check_need_prom_boot_chg
 * Description: Check to see if the current boot disk configuration will require
 *		a PROM or BIOS change.
 * Scope:	public
 * Parameters:	none
 * Return:	1	- PROM/BIOS change needed
 *		0	- No change needed
 */
int
check_need_prom_boot_chg(void)
{
	char	curdsk[MAXPATHLEN], exdsk[MAXPATHLEN];
	char	curstubdsk[MAXPATHLEN], exstubdsk[MAXPATHLEN];
	int	curdev, exdev;
	int	curstubpno, exstubpno;
	int	warn = -1;
	Disk_t  *dp = NULL;

	/*
	 * Don't print a warning about changing the BIOS when
	 * installing into a virtual block device (xpv)
	 */
	(void) DiskobjFindBoot(CFG_CURRENT, &dp);
	if (dp != NULL && disk_is_vbd(dp))
		return (0);

	(void) BootobjGetAttribute(CFG_CURRENT,
				BOOTOBJ_DISK, curdsk,
				BOOTOBJ_DEVICE, &curdev,
				BOOTOBJ_STUBBOOT_DISK, curstubdsk,
				BOOTOBJ_STUBBOOT_PARTNO, &curstubpno,
				NULL);
	(void) BootobjGetAttribute(CFG_EXIST,
				BOOTOBJ_DISK, exdsk,
				BOOTOBJ_DEVICE, &exdev,
				BOOTOBJ_STUBBOOT_DISK, exstubdsk,
				BOOTOBJ_STUBBOOT_PARTNO, &exstubpno,
				NULL);

	/*
	 * Compare the boot disks.  The first few cases concern the stub
	 * boot partition, which exists only on Intel.  Neither the CFG_CURRENT
	 * nor the CFG_EXIST stub boot partitions will be set on non-Intel
	 * machines, so we dispense with the IsIsa test when looking at
	 * stub boot partitions.
	 */
	if (exstubdsk[0] != '\0') {
		if (curstubdsk[0] != '\0') {
			/*
			 * Existing stub boot partition and current stub boot
			 * partition.  Compare the partitions.
			 */
			if (streq(exstubdsk, curstubdsk) &&
			    exstubpno == curstubpno) {
				warn = 0;
			} else {
				warn = 1;
			}
			...
			/*
			 * Existing stub boot partition, no current stub boot
			 * partition. Compare the existing partition with the
			 * current root disk.
			 */
			if (streq(exstubdsk, curdsk) &&
			    exstubpno == curdev) {
				warn = 0;
			} else {
				warn = 1;
	...
	} else if (curstubdsk[0] != '\0') {
		/*
		 * No existing stub boot partition, current stub boot partition.
		 * Compare the existing root disk with the current boot
		 * partition.
		 */
		if (streq(exdsk, curstubdsk) && exdev == curstubpno) {
			warn = 0;
		} else {
			warn = 1;
		}
	} else if (streq(curdsk, exdsk) &&
		    (IsIsa("i386") || curdev == exdev)) {
		/*
		 * No stub boot partitions, and the disks are the same (and for
		 * SPARC, the slice numbers are also the same).
		 */
		warn = 0;
	} else {
		/*
		 * No stub boot partitions, but the disks aren't the same (for
		 * SPARC it could also be that the slice numbers aren't the
		 * same)
		 */
		warn = 1;
	}
...
_check_bootability(void)
{
	 ...
	 * configuration
	 */
	if (check_need_prom_boot_chg()) {
		if (prom_changeable && prom_authorized) {
			if (IsIsa("i386")) {
				_add_error_msg(D_PROMRECONFIG,
					MSG0_BOOT_PROM_CHANGING_x86);
			} else {
				_add_error_msg(D_PROMRECONFIG,
					MSG0_BOOT_PROM_CHANGING_SPARC);
			}
			count++;
		} else {
			if (IsIsa("i386")) {
				_add_error_msg(D_PROMMISCONFIG,
					MSG0_BOOT_PROM_CHANGE_REQUIRED_x86);
			} else {
				_add_error_msg(D_PROMMISCONFIG,
					MSG0_BOOT_PROM_CHANGE_REQUIRED_SPARC);
			}
			count++;
		}
	}
...
_dup_slice_keys(Disk_t *dp)
{
	...
	WALK_SLICES(i) {
		...
		WALK_DISK_LIST(p) {
			 ...
			 * disks which aren't selected, or with disks that don't
			 * have Solaris partitions.
			 */
			if (p == dp || !disk_selected(p))
				continue;

			if (disk_fdisk_req(p) &&
			    !get_solaris_part(p, CFG_CURRENT))
				continue;
 ...
 * Description: On i386 systems that don't use LBA, check to see that the
 *		Solaris partition begins before the BIOS 1023 cylinder.
 * Scope:	private
 * Parameters:	dp	- non-NULL disk structure pointer
 * Return:	# >= 0	- number of errors/warnings which occurred
 */
 ...
 * Description: On i386 systems that don't use LBA, check to see that slices 0,
 *		8, and 9 all end before physical drive cylinder 1023 (due to
 *		BIOS limitations in the boot code).
 * Scope:	private
