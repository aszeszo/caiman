...
#pragma ident	"@(#)store_initdisk.c	1.46	07/10/09 SMI"
...
static int	FindLBAPropCallback(di_node_t node, void *found);
static int	InitializePromGeom(Disk_t *);
static int	is_it_a_disk(int, Disk_t *);
...
#define	LBA_ACCESS_PROP "lba-access-ok"	/* LBA access property name */
...
/*
 * Function:	FindLBAProp
 * Description:	Look for the `lba-access-ok' property in the disk node
 *		corresponding to this disk device.  If this property
 *		is found, the disk is free of the 1023-cylinder root
 *		restrictions.
 *
 * Scope:	private
 * Parameters:	device	[RO, *RW]
 *		Pointer to buffer containing valid device name
 * Return:	1	- lba-access-ok property found
 *		0	- property not found
 */
int
FindLBAProp(char *device)
{
	di_node_t node;
	char nodepath[PATH_MAX]; /* Eventually the path to the disk node */
	char *c;
	int found;

	/* Chase the path down to the /devices entry */
	if (realpath(device, nodepath) == NULL) {
	    return (0);
	}

	/* Strip off a ^/devices, if there is one */
	if (strncmp(nodepath, "/devices", 8) == 0) {
		(void) memmove(nodepath, (nodepath + 8),
		    strlen(nodepath) - 8 + 1);
	}

	/* Strip off the trailing slice specifier (re: :.*$), if any */
	if ((c = strrchr(nodepath, ':')) != NULL) {
		*c = '\0';
	}

	/* nodepath now (hopefully) contains the path to the disk node */
	if ((node = di_init(nodepath, DINFOPROP)) == DI_NODE_NIL) {
		return (0);
	}

	/* Walk the node, looking for the LBA property */
	found = 0;
	di_walk_node(node, DI_WALK_CLDFIRST, &found, FindLBAPropCallback);

	/* Free the tree, save the whales */
	di_fini(node);
...
DiskobjInitialize(char *name)
{
	...
	/* Initialize prom-specific disk geometry */
	if (disk_okay(dp))
		(void) InitializePromGeom(dp);
...
FdiskobjInit(Disk_t *dp)
{
	...
	if (GetSimulation(SIM_SYSDISK) == 0) {
		...
		if (IsIsa("ppc")) {
			...
			(void) snprintf(cmd, sizeof (cmd),
			    "/sbin/fdisk -n -R -v -W - %s 2>&1", device);
		}
 ...
 *		returns `0'. With long name such as world wide name, we
 * 		check whether ity ends with s2.
 * Scope:	private
 * Parameters:	name	- [RO, *RO]
...
NameIsWholeDisk(char *name)
{
	...
	/*
	 * Check whether the disk name ends with s2
	 */

	if (strncmp(name+strlen(name)-2, "s2", 2) == 0)
		return (1);
	else
		return (0);
}
 ...
 *		c0d0p1). With World wide Name, we cannot check the
 * 		whole string, we will check the last two characters.
 *		They should be in the form 'sX', where X is a digit
 *		between 0 and 7. For x86, World Wide Name doesn't
 *		apply right now. So check the whole string.
 * Scope:	private
 * Parameters:	name	- [RO, *RO]
 *			  String containing disk name to be evaluated;
 *			  this is the device name and not the path to
 *			  the device
 * Return:	0	- the string is not a disk name
 *		1	- the string does represent a disk name
 */
static int
NameIsDiskDevice(char *name)
{
	...
	if ((name[strlen(name)-2] == 's') && isdigit(name[strlen(name)-1]))
		return (1);
	else {
		/*
		 * For X86 disks, MPXIO doesn't apply. So retain the
		 * check to validate whether the disk is of the form cXdXpX
		 */
...
DiskobjInitControllerLive(Disk_t *dp)
{
	...
	/*
	 * Check whether the /dev/rdsk/entry is a disk. We need to
	 * eliminate CDROM and Floppy drives.
	 */
	if ((n = is_it_a_disk(fd, dp)) != 0) {
		(void) close(fd);
		/*
		 * In the case unknown controller, return 0 since
		 * the /dev/rdsk/entry could be a disk.
		 * We are retaining the historic behavior of the code
		 */
		if (n == 1) {
			return (0);
		} else {
	...
	/* bail if we didn't get geometry */
	if (n < 0) {
		/* cannot format IDE disks */
		if (disk_ctype(dp) == DKC_DIRECT)
			disk_state_set(dp, DF_CANTFORMAT);
	...
	/* Look for the `lba-access-ok' property on the disk node */
	if (IsIsa("i386") && FindLBAProp(device)) {
		disk_state_set(dp, DF_USESLBA);
	}
	...
	/* check if the geometry is bogus */
	if (dkg.dkg_pcyl == 0 || dkg.dkg_nsect == (u_short)0) {
		/* cannot format IDE disks */
		if (disk_ctype(dp) == DKC_DIRECT)
			disk_state_set(dp, DF_CANTFORMAT);
		return (0);
...
/*
 * Function:	InitializePromGeom
 * Description:	Figures out any prom-specific geometry information, and
 *		initializes a disk with it.
 *
 *		The mapping between PROM revision levels and the
 *		furthest out the root slice can reach on a disk
 *		is as follows (for sun4m only):
 *
 *         ------------            ---------------------------------
 *         < 2.9                   up to the first 1GB of the drive
 *         2.9 - 2.24              up to the first 2GB of the drive
 *         2.25 or greater         up to the first 4GB of the drive
 *
 *         3.0 or greater          anywhere
 *
 * Scope:	private
 * Parameters:	dp	[RW]
 *			Disk which we are initializing
 * Return:		SUCCESS - successfully figured out prom and geometry
 *		anythign else - Couldn't figure out prom information
 */
static int
InitializePromGeom(Disk_t *dp)
{
	char	*rev;
	char	*machine;
	char	*rev_tok;
	...
	/* intialize the limitation to the max value (no limit) */
	disk_geom_lrootcyl(dp) = disk_geom_lcyl(dp);

	/* first get the revision level */
	rev = xstrdup(get_prom_revision());
	...
	/* only sun4m machines have proms which have limitations */
	if (!streq(machine, "sun4m")) {
		return (SUCCESS);
	}

	/* tokanize the version */
	rev_tok = strtok(rev, ".");
	if (rev_tok == NULL || (!is_allnums(rev_tok))) {
		return (-1);
	}
	...
	if (atoi(rev_tok) < 2) {
		/* less than 2.0 */
		gb_limit = 1;
	} else if (atoi(rev_tok) > 2) {
		/* 3.0 or higher, no limit, nothing to do */
		return (SUCCESS);
	} else {
		/* 2.something.  continue parse */
		rev_tok = strtok(NULL, ".");
		if (rev_tok == NULL || (!is_allnums(rev_tok))) {
			/* must have been "2", treat it like "2.0" */
			gb_limit = 1;
		} else {
			if (atoi(rev_tok) < 9) {
				/* [2.0 - 2.9) */
				gb_limit = 1;
			} else if ((atoi(rev_tok) >= 9) &&
			    (atoi(rev_tok) <= 24)) {
				/* [2.9 - 2.24] */
				gb_limit = 2;
			} else {
	...
	/*
	 * calculate the last cylinder beyond which the root slice
	 * cannot exist.
	 *
	 * if largest cylinder for / slice is beyond end of disk,
	 * then just set the limit to the end of the disk
	 */
	disk_geom_lrootcyl(dp) = MIN(disk_geom_lcyl(dp),
	    mb_to_cyls(dp, gb_to_mb(gb_limit)));

	return (SUCCESS);
}

/*
 * Function:	FindLBAPropCallback
 * Description:	A callback used by FindLBAProp (via di_walk_node) to search
 *		a node from the device tree for the LBA property.  The search
 *		is terminated when the node is found.
 *
 * Scope:	private
 * Parameters:	node	[RO]
 *			Node on device tree
 *		found	[RO, *RW]
 *			flag indicating whether or not the property was found
 * Return:	DI_WALK_CONTINUE - property not found - continue search
 *		DI_WALK_TERMINATE - property found - terminate search
 */
static int
FindLBAPropCallback(di_node_t node, void *found)
{
	di_prop_t prop;

	for (prop = di_prop_next(node, NULL); prop;
		prop = di_prop_next(node, prop)) {
		    if (strcmp(di_prop_name(prop), LBA_ACCESS_PROP) == 0) {
			*((int *)found) = 1;
...
SdiskobjInitLive(Disk_t *dp)
{
	...
	WALK_SLICES(slice) {
		 ...
		 */
		if (slice == ALT_SLICE &&
		    (_disk_is_scsi(dp) || disk_is_vbd(dp))) {
			size = 0;
		} else if ((over = vtoc.v_part[slice].p_start % one_cyl(dp))
			> 0) {
...
/*
 * Functions:	is_it_a_disk
 * Description: Check whether /dev/rdsk/entry is valid disk and not a
 *		removable device like floppy or cdrom. Even though USB
 *		devices are removable, we should allow the USB disks
 * Scope:	private
 * Parameters:	fd 	File descriptor to the device
 * 		dp  	Disk pointer
 * Return:	 0	- if the entry is valid
 *		-1	- if the entry is invalid
 *		 1	- Return if the controller is unknown
 *			- Preserve historical bahavior
 */
static int
is_it_a_disk(int fd, Disk_t *dp)
{
	...
	/*
	 * Attempt to read the controller configuration info on the disk.
	 * If it fails, treat the device as though it was NOT a disk.
	 */
	...
	/*
	 * If it is identified as a CDROM, treat the device as NOT A DISK.
	 */
	...
	disk_state_unset(dp, DF_BADCTRL);
	disk_ctype_set(dp, cinfo.dki_ctype);

	/*
	 * The unknown controller type is handled as success
	 * historically.
	 */
	if (cinfo.dki_ctype == DKC_UNKNOWN) {
		return (1);
	}

	/*
	 * The USB devices are listed under /dev/rdsk/
	 * Get the media info and eliminate all devices which are
	 * not of the type DK_FIXED_DISK (CDROM, floppy etc.)
	 */
	if ((ioctl(fd, DKIOCGMEDIAINFO, &mediainfo) < 0) &&
	    (errno == ENOTTY)) {
		/*
		 * DKIOCGMEDIAINFO is not implemented for Solaris 7. It is
		 * not implemeted for some devices in Solaris 8 and Solaris 9.
		 * Use DKIOCREMOVABLE instead.
		 */
		...
		/*
		 * Ignore the removable devices and accept the
		 * non-removable devices like disks.
		 */
		if (removable) {
			return (-1);
		} else {
	...
	/*
	 * Ignore the removable devices. USB storage devices will have
	 * the media type DK_FIXED_DISK whether Solaris treats them
	 * removable or non-removable. Solaris treats the USB storage
	 * devices as removable devices by default.
	 */

	if (mediainfo.dki_media_type != DK_FIXED_DISK) {
		return (-1);
	}

	/*
	 * To create a SOLARIS fdisk partition we need atleast 30 MB
	 * So including the overhead, we check whether the disk is
	 * atleast 32 MB.
	 */

	if (IsIsa("i386") && strstr(cinfo.dki_cname, "usb")) {
		/* Convert bytes to MB */
		if (((mediainfo.dki_lbsize *
			mediainfo.dki_capacity)/ (1024 * 1024))
				< MIN_DISK_SIZE) {
			return (-1);
		}
	}
