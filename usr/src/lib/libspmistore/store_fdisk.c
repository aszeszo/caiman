...
#pragma ident	"@(#)store_fdisk.c	1.19	07/10/09 SMI"
...
int		get_diagnostic_part(Disk_t *, Label_t);
...
static int	PartobjFindId(Disk_t *, Label_t state, int);
...
adjust_part_starts(Disk_t *dp)
{
	...
	/* set the starting sector comparitor to an invalid value */
	start = -1;
	...
	WALK_PARTITIONS(i) {
		...
		if (part_preserved(dp, i) || part_stuck(dp, i)) {
			...
			part = Partobj_Id(CFG_CURRENT, dp, i);
...
/*
 * Function:	fdisk_parts_avail
 * Description:	Calculate the number of unused partition table entries for
 *		the indicated state and disk.
 * Scope:	public
 * Parameters:	dp	- non-NULL disk structure pointer
 * Return:	0<=#<=4	  - the number of free partition table entries
 */
int
fdisk_parts_avail(Disk_t *dp)
{
	int free = 0;
	int p;
 ...
 * Description: Retrieve the partition index number for the Solaris2 (SUNIXOS2)
 *		partition in the current, committed, or existing fdisk.
 *		If Solaris2 not found search for Solaris (SUNIXOS) to work with
 *		systems with older version of Solaris.
 * Scope:	public
 * Parameters:	dp	- non-NULL disk structure pointer
...
get_solaris_part(Disk_t *dp, Label_t state)
{
	...
	ret = PartobjFindId(dp, state, SUNIXOS2);
	/*
	 * cannot find Solaris 2 partition, search for Solaris
	 */
	if (ret == 0) {
		return (PartobjFindId(dp, state, SUNIXOS));
	}
	return (ret);
}

/*
 * Function:	get_stubboot_part
 * Description:	Retrieve the partition number for the stub boot (X86BOOT)
 *		partition in the current, committed, or existing fdisk.
 * Scope:	public
 * Parameters:	dp	- non-NULL disk structure pointer
 *		state	- specify which fdisk configuration should be
 *			  searched (CFG_CURRENT, CFG_COMMIT, or CFG_EXIST)
 * Return:	0	- no stub boot partition found, or 'state' invalid
 *		1<=#<=4	- partition index for stub boot partition (1 - 4)
 */
int
get_stubboot_part(Disk_t *dp, Label_t state)
{
	return (PartobjFindId(dp, state, X86BOOT));
}

/*
 * Function:	get_diagnostic_part
 * Description:	Retrieve the partition number for the Diagnostic (DIAGPART)
 *		partition in the current, committed, or existing fdisk.
 * Scope:	public
 * Parameters:	dp	- non-NULL disk structure pointer
 *		state	- specify which fdisk configuration should be
 *			  searched (CFG_CURRENT, CFG_COMMIT, or CFG_EXIST)
 * Return:	0	- no diagnostic partition found, or 'state' invalid
 *		1<=#<=4	- partition index for diagnostic partition (1 - 4)
 */
int
get_diagnostic_part(Disk_t *dp, Label_t state)
{
	return (PartobjFindId(dp, state, DIAGPART));
}
...
part_overlaps(Disk_t *dp, int part, int start, int size, int **olpp)
{
	...
	WALK_PARTITIONS(f) {
		...
		if ((Partobj_Id(CFG_CURRENT, dp, f) != UNUSED) &&
		    ((fstart >= start && fstart < pend &&
		    part_size(dp, f) > 0) ||
		    (start >= fstart && start < (fstart + part_size(dp, f))))) {
...
max_size_part_hole(Disk_t *dp, int part)
{
	...
	if (dp == NULL || !disk_okay(dp) ||
	    !disk_selected(dp) || !valid_fdisk_part(part)) {
		return (0);
	}
	...
	/*
	 * Find the first used partition before this one.  If the end
	 * of that partition lies before the first cylinder on the
	 * disk, we must reduce the possible size of this partition
	 * by the difference, because new partitions must lie on
	 * cylinder boundaries in spmi, and cannot be placed before the
	 * first cylinder limit on the disk.
	 */
	for (p = part - 1; p > 0; p--)
		if (Partobj_Id(CFG_CURRENT, dp, p) != UNUSED)
			break;
	...
	/*
	 * If there are no valid partitions before this one, then the
	 * starting sector will be at the first cylinder, and
	 * therefore the size has already been computed correctly by
	 * _fdisk_segment_size, which takes into account the fact that
	 * partitions can't start before the first cylinder boundary.
	 * So we don't have to adjust the returned size.  Otherwise we
	 * do.
	 */
	if (p != 0) {
		if (valid_fdisk_part(p) &&
		    (part_startsect(dp, p) + part_size(dp, p)) <
 ...
 *		id	- fdisk partition id specifier:
...
set_part_attr(Disk_t *dp, int part, int id, int active)
{
	...
	if (id != GEOM_IGNORE && id != Partobj_Id(CFG_CURRENT, dp, part)) {
		...
		case	SUNIXOS:
			 ...
			 *       if this partition is not the boot device and
			 *       there's no x86 boot partition.
			 *	   return D_BOOTFIXED
			 *	 fi
			 *     fi
			 *   else
			...
			if (IsIsa("i386")) {
				(void) BootobjGetAttribute(CFG_CURRENT,
					    ...
					    BOOTOBJ_STUBBOOT_DISK, stubdisk,
					    NULL);
			...
			if (PartobjFindId(dp, CFG_CURRENT, id))
				return (D_DUPMNT);
		...
		case X86BOOT:
			/*
			 * if this is Intel
			 *   if this is the stubdisk stored in the boot object
			 *     if the partition number hasn't already been set
			 *       save the partition ID
			 *     else
			 *       return D_BOOTFIXED
			 *     fi
			 *   else if no stubdisk has been set
			 *     save this disk and partition ID
			 *   else
			 *     return D_BOOTFIXED
			 *   fi
			 * fi
			...
			if (IsIsa("i386")) {
				(void) BootobjGetAttribute(CFG_CURRENT,
					    BOOTOBJ_STUBBOOT_DISK, stubdisk,
					    BOOTOBJ_STUBBOOT_PARTNO,
								&stubpartno,
					    NULL);

				if (streq(disk_name(dp), stubdisk)) {
					if (stubpartno == 0 ||
					    stubpartno == part) {
						(void) BootobjSetAttribute(
							CFG_CURRENT,
				...
				} else if (streq(stubdisk, "")) {
					(void) BootobjSetAttribute(
						CFG_CURRENT,
						BOOTOBJ_STUBBOOT_DISK,
								disk_name(dp),
						BOOTOBJ_STUBBOOT_PARTNO, part,
						NULL);
				} else {
					return (D_BOOTFIXED);
				}
		...
		case   UNUSED:
			 ...
			 *     fi
			 *   fi
			 *   if this is the current stub boot disk and partition
			 *     set the stub boot disk and partition to undefined
			 *   fi
			 * fi
			 */
			if (IsIsa("i386")) {
				...
				(void) DiskobjFindStubBoot(CFG_CURRENT, &bdp,
								&stubpartno);
				if (dp == bdp) {
					if (part == stubpartno) {
						BootobjSetAttribute(CFG_CURRENT,
						    BOOTOBJ_STUBBOOT_DISK, "",
						    BOOTOBJ_STUBBOOT_PARTNO, 0,
						    NULL);
					}
				}
			}
		...
		default:
			...
			p = Partobj_Id(CFG_CURRENT, dp, part);
			if (p == SUNIXOS || p == SUNIXOS2) {
				part_geom_firstcyl(dp, part) = 0;
				part_geom_lcyl(dp, part) =
	...
	if (active != GEOM_IGNORE &&
	    active != Partobj_Active(CFG_CURRENT, dp, part)) {
		if (active == ACTIVE) {
			WALK_PARTITIONS(i)
...
/*
 * Function:	PartobjFindId
 * Description: Determine if the current fdisk configuration has a partition
 *		with a specific partition id.
 * Scope:	private
 * Parameters:	dp	- non-NULL disk structure pointer
 * Return:	0	- no partition with the specified id found,
 *			  or 'state' invalid
 *		1<=#<=4 - index for partition with specified id (1 - 4)
 */
static int
PartobjFindId(Disk_t *dp, Label_t state, int id)
{
	int	p;
...
_shift_part_entries(Disk_t *dp, int part)
{
	...
	(void) memcpy(&tmp,
	    Partobj_Addr(CFG_CURRENT, dp, part + 1), sizeof (Part_t));
	(void) memcpy(Partobj_Addr(CFG_CURRENT, dp, part + 1),
	    Partobj_Addr(CFG_CURRENT, dp, part), sizeof (Part_t));
	(void) memcpy(Partobj_Addr(CFG_CURRENT, dp, part),
	    &tmp, sizeof (Part_t));
