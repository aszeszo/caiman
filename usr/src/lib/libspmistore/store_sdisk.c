...
filesys_preserve_ok(char *fs)
{
	...
	if (get_install_type() == CMNInitialInstall) {
 ...
 *		scyl	- required starting cylinder for any reported hole, or
 *			GEOM_IGNORE to have unconstrained start cylinder.
 *		ecyl	- required maximum ending cylinder for hole returned,
 *			or GEOM_IGNORE for no constraint
 * Returns:	0	- no space available
 *		#	- number of unallocated usable blocks
 */
int
sdisk_max_hole_size(Disk_t *dp, int scyl, int ecyl)
{
	int	s;
	int	sstart = 0;
	int	send;
	...
	while (sstart <
	    ((ecyl == GEOM_IGNORE) ? sdisk_geom_lcyl(dp) : ecyl)) {
		/*
		...
		if (!valid_sdisk_slice(s)) {
			...
			WALK_SLICES(f) {
				if (Sliceobj_Size(CFG_CURRENT, dp, f) > 0 &&
					...
					send =
					    Sliceobj_Start(CFG_CURRENT, dp, f);
			}

			if (((scyl == GEOM_IGNORE) || (scyl == sstart)) &&
			    ((send - sstart) > hole)) {
				hole = send - sstart;
			}
			sstart = send;
...
sdisk_space_avail(Disk_t *dp)
{
	...
	if (dp == NULL || !disk_okay(dp) ||
	    sdisk_geom_null(dp) ||
	    SdiskobjIsIllegal(CFG_CURRENT, dp))
		return (0);
		...
		char *name, int instance, int match)
{
	...
	if (disk != NULL) {
		...
		if (disk_fdisk_req(disk) &&
		    !get_solaris_part(disk, CFG_CURRENT))
			return (NULL);
...
coreSliceobjSetSize(Label_t state, Disk_t *dp, int slice, int newsize)
{
	...
	if (newsize == GEOM_REST) {
		blks = sdisk_max_hole_size(dp, GEOM_IGNORE, GEOM_IGNORE);
		if (blks == 0)
			return (D_NOSPACE);
	} else {
		...
		char *use, int instance)
{
	 ...
	 */
	if (instance >= 0) {
		if (SliceobjFindUse(state, NULL, use, instance, 1) != NULL)
			return (D_DUPMNT);
	}
...
coreSliceobjGetAltsectorSize(const Disk_t *dp)
{
	...
	if (_disk_is_scsi(dp) || disk_is_vbd(dp))
		return (0);
