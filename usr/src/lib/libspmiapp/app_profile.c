...
#pragma ident	"@(#)app_profile.c	1.33	07/10/09 SMI"
...
/* longest length of error message */
#define	MAXMSGLEN	1024
#define	POS(x)		(((x) > 0) ? (x) : 0)

/* private prototypes */

static int	_app_set_slice(char *, int, int, char *,
		char *, int, int, char **);
static char	*_app_fit_slice(Profile *, Disk_t *, int, int, char *);
...
configure_sdisk(Profile *prop)
{
	...
	*fsp = new;
	 ...
	 * Process metadb
	 */
	if ((status = setup_metadb_disk(prop)) != D_OK)
		return (status);
	/*
	...
	WALK_LIST(fsf, *fsp) {
		if (fsf->is_mirror) {
			if ((status = setup_mirror_disk(prop, fsf)) != D_OK)
				return (status);
		} else {
			if ((status = app_config_slice(prop, fsf)) != D_OK)
				return (status);
		}
	}
	...
	if (DISKPARTITIONING(prop) == LAYOUT_DEFAULT) {
		...
		/*
		 * If the user has no preference where swap should go, let's
		 * force swap to start at the beginning of the disk if there
		 * is a disk with enough space at the beginning so future
		 * upgrades using the Installation CD/DVD will have maximum
		 * flexibility during upgrade
		 */
		if (SliceobjFindUse(CFG_CURRENT, NULL, SWAP,
		    VAL_UNSPECIFIED, FALSE) == NULL) {
			if ((swapdevice = find_swap(GEOM_BEGINNING)) != NULL) {
				/*
				 * Found a disk that can hold swap at the
				 * beginning.  Let's mark the SWAP Resource
				 * object as being required to start at
				 * the beginning of that disk
				 */
				if ((res = ResobjFind(SWAP, 0)) != NULL) {
					dp = find_disk(swapdevice);
					firstavail = sdisk_geom_firstcyl(dp);
					(void) ResobjSetAttributePriv(res,
					    RESOBJ_DEV_EXPLSTART,  firstavail,
					    NULL);
				}
			}
		}
 ...
 */
int
app_config_slice(Profile *prop, Storage *fsp)
{
	ResobjHandle	res;
	int 		status;
	int		explicit = 0;
	...
	/*
	 * If mirror_name is not null, then we are configuring mirror device
	 * so display the voulme name
	 */
	if (fsp->mirror_name) {
		write_status(LOGSCR, LEVEL1|LISTITEM,
			MSG3_METADEVICE_CONFIGURE,
			fsp->mirror_name,
			fsp->name,
			fsp->dev);
	} else {
	 ...
	 */
		...
		/* put user-specified swap at beginning if possible */
		if (strcmp(fsp->name, SWAP) == 0) {
			start = GEOM_BEGINNING;
		} else if (strcmp(fsp->name, "SWAP_MIRROR") == 0) {
			/*
			 * If the swap is mirrored, we want to name only the
			 * first slice as swap to prevent the second submirror
			 * added by the system as swap but created at the 
			 * beginning of disk like swap.
			 */
			start = GEOM_BEGINNING;
			fsp->name = strdup("");
		} else {
	 ...
	 */
		...
		*cp++ = '\0';
		...
		/*
		 * if this is the root slice specifier, make
		 * sure it's not beyond any platform-specific limits.
		 *
		 * Neither the start or ending cylinder can be beyond the
		 * maximum allowable cylinder.
		 *
		 * The start cylinder is specified explicitly by the user,
		 * and is an index assumed to begin at 0.  For example,
		 * if you specify a start cylinder of 5, you are really
		 * starting at cylinder 6.
		 *
		 * The end cylinder is computed as start cyl + size.
		 * This is the number of cylinders from the beginning
		 * of the disk that the range of cylinders will extend
		 * to.  For example, if you say "start at cylinder 5,
		 * extend for 5 cylinders", you will occupy cylinders
		 * 6 through 10 (index 5-9).
		 *
		 * If the cylinder range extends beyind the max # of
		 * cylinders that the root partition can exist on,
		 * we fail.
		 */
		if (streq(fsp->name, ROOT)) {
			if ((start >= disk_geom_lrootcyl(dp)) ||
			    ((start + atoi(cp))  > disk_geom_lrootcyl(dp))) {
				write_notice(ERRMSG,
				    MSG2_CYL_INVALID_FOR_PLAT,
				    start, start + atoi(cp),
				    disk_geom_lrootcyl(dp));
				return (D_FAILED);
			}
		}
	}
	 ...
	 */
		...
		if (streq(fsp->name, SWAP)) {
			if (ci_streq(fsp->dev, "any")) {
				/*
				 *try to start it at the front of a
				 * disk
				 */
				device = find_swap(GEOM_BEGINNING);
				if (device != NULL) {
					/*
					 * found room at beginning, so
					 * make it official */

					dp = find_disk(device);
					start = sdisk_geom_firstcyl(dp);
				} else {
					/* can't be at beginning */
					device = find_swap(GEOM_IGNORE);
				}
			}
			else
				device = fsp->dev;
			...
			if (_app_fit_slice(prop, dp, GEOM_IGNORE,
			    size, fsp->name) == NULL)
	...
	if (device == NULL) {
		if (ci_streq(fsp->dev, "any")) {
			/*
			 * try to find a disk that has an open slice
			 * which can hold the required storage, at the
			 * requested start point.
			 */
			device = _app_fit_slice(prop, NULL, start, size,
			    fsp->name);
			if ((strcmp(fsp->name, SWAP) == 0) &&
			    (device == NULL) && (start == GEOM_BEGINNING)) {
				/*
				 * If we are configuring a swap slice,
				 * try again if the hole wasn't
				 * available at the start of the disk.
				 * If this suceeds, then we must
				 * re-set the start value to
				 * GEOM_IGNORE as the swap slice was
				 * determined not to fit at the front
				 * of any disk.
				 */
				device = _app_fit_slice(prop, NULL,
				    GEOM_IGNORE, size, fsp->name);
				if (device != NULL) {
					start = GEOM_IGNORE;
				}
			}
	...
	/*
	 * if this is the root filesystem, set the last available cylinder
	 * to be the limit of where the root filesystem can be.
	 */
	if (streq(fsp->name, ROOT)) {
		lastavail = disk_geom_lcyl(find_disk(device));
		disk_geom_lcyl(find_disk(device)) =
		    disk_geom_lrootcyl(find_disk(device));
	...
	if ((status = _app_set_slice(device, start, size,
	    ...
	    fsp->preserve, &errmsg)) != D_OK) {

		/* restore last available cylinder to its original value */
		if (streq(fsp->name, ROOT)) {
			disk_geom_lcyl(find_disk(device)) = lastavail;
		}

		/*
		 * swap might not fit at the front; try any
		 * other locations if possible
		 */
		if ((strcmp(fsp->name, SWAP) == 0) &&
		    (start == GEOM_BEGINNING)) {
			if ((status = _app_set_slice(device, GEOM_IGNORE, size,
			    fsp->name, fsp->mntopts, explicit,
			    fsp->preserve, &errmsg)) != D_OK) {
				write_notice(ERRMSG, errmsg);
				return (status);
			}
		} else {
			write_notice(ERRMSG, errmsg);
 ...
 *	errmsg - if not NULL, will hold static error message if
 *	D_OK is not returned.
 * Return:
 *	D_OK		- set successful
 *	D_NODISK	-
 *	D_FAILED	- set failed
...
_app_set_slice(char *device, int start, int size,
			char *mnt, char *opts, int explicit,
			int preserve, char **errmsg)
{
	Disk_t		*dp;
	...
	int		reqstart;
	int		origstuck;
	static char	errbuf[MAXMSGLEN];

	if ((dp = find_disk(device)) == NULL) {
		snprintf(errbuf, MAXMSGLEN,
		    "(_app_set_slice) %s",
		    library_error_msg(D_NODISK));
		if (errmsg != NULL) *errmsg = errbuf;
		return (D_FAILED);
	}
	...
	/*
	 * resolve GEOM_BEGINNING into actual starting cylinder
	 * for requested disk
	 */
	if (start == GEOM_BEGINNING) {
		/* take into account alternate sector slice if present */
		if (ALT_SLICE < numparts &&
		    slice_size(dp, ALT_SLICE) > 0 &&
		    slice_start(dp, ALT_SLICE) == 1) {
			reqstart = slice_start(dp, ALT_SLICE) +
			    blocks_to_cyls(dp,
				slice_size(dp, ALT_SLICE));
		} else {
			reqstart = sdisk_geom_firstcyl(dp);
		}
	} else {
		reqstart = start;
	}
	 ...
	 */
	if (reqstart != GEOM_IGNORE) {
		/*
		 * remember original "stuck" status in case we
		 * have to reset it after a failure
		 */
		origstuck = slice_stuck(dp, slice);
	...
	if (status == D_OK &&
		 ...
		 */
		if (reqstart != GEOM_IGNORE) {
			if ((n = slice_overlaps(dp, -1, reqstart,
						size, &olp)) != 0) {
				for (i = 0; i < n; i++) {
			...
			/* append extra detail to error message */
			snprintf(errbuf+strlen(errbuf),
			    POS(MAXMSGLEN - strlen(errbuf)),
			    ": %s (%s)",
			    library_error_msg(status),
			    make_slice_name(disk_name(dp), slice));
		}
	...
	if (status == D_OK) {
		...
		/* Restore the stuck status to what it was before */
		if (reqstart != GEOM_IGNORE) {
			if (origstuck) {
				slice_stuck_on(dp, slice);
			} else {
 ...
 *	slices to hold swap.
 * Parameters:
 *	scyl	- required starting cylinder, GEOM_IGNORE or
 *		GEOM_BEGINNING or an explicit starting cylinder #.
 * Return:
...
find_swap(int scyl)
{
	...
	WALK_DISK_LIST(dp) {
		if (!disk_selected(dp) || sdisk_geom_null(dp))
			continue;
		 ...
		 * as a constraint
		 */
		if (scyl == GEOM_BEGINNING) {
			reqstart = sdisk_geom_firstcyl(dp);
		} else {
			reqstart = scyl;
		}

		if (sdisk_max_hole_size(dp, reqstart, GEOM_IGNORE) >=
 ...
 *		  disks
 *	start	- starting cylinder, or GEOM_IGNORE.  If this is GEOM_BEGINNING,
 *		the allocation will attempt to be found at the beginning of
 *		the disk.
 ...
 */
static char *
_app_fit_slice(Profile *prop, Disk_t *disk, int start, int size, char *mnt)
{
	Disk_t 	*dp;
	Disk_t 	*bdp;
	char	*device = NULL;
	...
	if (disk != NULL) {
		...
		/*
		 * if we are fitting /, observe any root slice size
		 * limitations
		 */
		if (streq(mnt, ROOT)) {
			end = disk_geom_lrootcyl(dp);
		} else {
			end = GEOM_IGNORE;
		}
		...
		if (disk_selected(dp) &&
				...
				sdisk_max_hole_size(dp, start, end) >= size)
			device = make_slice_name(disk_name(dp), slice);
	} else {
		/*
		...
		if (streq(mnt, ROOT)) {
			if (ROOTDEVICE(prop) != NULL) {
				...
				/*
				 * if we are fitting /, observe any root
				 * slice size limitations
				 */
				if (streq(mnt, ROOT)) {
					end = disk_geom_lrootcyl(bdp);
				} else {
					end = GEOM_IGNORE;
				}

				if (!sdisk_is_usable(bdp) ||
				sdisk_max_hole_size(bdp, start, end) < size)
					return (NULL);
		...
		if (device == NULL) {
			WALK_DISK_LIST(dp) {
				...
				if ((slice = _app_find_slice(dp, mnt)) >= 0 &&
					sdisk_max_hole_size(dp, start, end) >=
							size) {
