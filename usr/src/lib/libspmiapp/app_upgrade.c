...
#pragma ident	"@(#)app_upgrade.c	1.30	07/10/09 SMI"
...
/*
 * list of non-upgradeable zones. Allocated when bad zones are found
 * and restored to heap when error message is displayed
 */
static StringList *pBadZoneList = NULL;
...
SliceGetUpgradeable(UpgOs_t **slices)
{
	...
	/*
	 * Transfer the data from the OSList to the app level data
	 * structure.
	 */
	*slices = (UpgOs_t *) xcalloc(sizeof (UpgOs_t) * (count + 1));
	...
	count = 0;
	LL_WALK(oslist, curnode, oli, err) {
		/*
		 * If there's an error, LL_WALK will run infinitely.
		 * Uhm... OK...
		 */
		if (err != LLSuccess)
			break;
		...
		slice_array[count].slice = xstrdup(oli->rootslice);
		slice_array[count].stub = oli->stubdevice ?
				xstrdup(make_device_name(oli->stubdevice,
							oli->stubpartno)) :
				NULL;
		slice_array[count].svminfo = oli->svminfo;
		slice_array[count].svmstring = xstrdup(oli->svmstring);
...
SliceFreeUpgradeable(UpgOs_t *slices)
{
	...
	for (i = 0; SliceGetTotalNumUpgradeable(slices); i++) {
		...
		if (slices[i].stub)
			free(slices[i].stub);
		if (slices[i].svminfo)
			spmi_svm_free(slices[i].svminfo);
	}
	free(slices);
}
...
SliceSetSelectedFromBootObj(UpgOs_t *slices)
{
	...
	found = FALSE;
	(void) snprintf(slice_name, sizeof (slice_name), "%s%c%d", diskname,
	    device_type, device);
...
/*
 * Function:	CheckZonesUpgradeableFromSlice
 * Description: Mount the given slice and call NonUpgradeableZonelist
 *		to get the list of Solaris zones on that slice
 * 		which are not upgradeable
 *
 * Scope:	public
 * Parameters:	device		- disk slice that contains root file system
 ...
 */
boolean_t
CheckZonesUpgradeableFromSlice(char *device, StringList **badZoneList)
{
	boolean_t	zones_exist = B_FALSE;
	...
	*badZoneList = NULL;
	 ...
	 */
	if (mount_and_add_swap(device, NULL) == 0) {
		if ((zones_exist = z_non_global_zones_exist())) {
			*badZoneList = NonUpgradeableZonelist();
		}
	}
...
AppUpgradeInitSw(unsigned int *state)
{
	...
	if ((prod = get_current_product()) != (Module *) NULL) {
		...
		/*
		 * clients do not get upgraded with this commented out.
		 */
		if (upgrading_clients()) {
		    (void) load_clients();
		}
	...
	void *parent_reinit_data)
{
	...
	/* check to see if slice has been converted to svm list */
	if (slice->svminfo != NULL &&
	    slice->svminfo->md_comps[0] != NULL &&
	    slice->svminfo->count > 0) {
		/* it has, use the first one in the list here */
		slice->slice = xstrdup(slice->svminfo->md_comps[0]);
	}
	...
	err = mount_and_add_swap(slice->slice, slice->stub);
	if (err != SUCCESS) {
		return (AppParentProcessSliceError(slices, slice, *state,
			err, confirm_exit_func));
	}
	...
	/*
	 * check for bad zones.  This check must be done after
	 * the root slice is mounted.
	 */
	if ((pBadZoneList = NonUpgradeableZonelist()) != NULL) {
		return (AppParentProcessSliceError(slices, slice, *state,
			ERR_ZONE_FAIL, confirm_exit_func));
	}
	...
	if (resume_state != UpgradeResumeNone) {
		 ...
		 */
		slice_str_len = strlen(UPG_RECOVER_QUERY) +
		    strlen(slice->slice) +
		    strlen(slice->release) +
		    1;
		str = (char *)xmalloc(slice_str_len);
		(void) snprintf(str, slice_str_len, UPG_RECOVER_QUERY,
	...
	int err, void (*confirm_exit_func)(void))
{
	 ...
	 */
	if (slice->svminfo != NULL && slice->svminfo->count > 0 &&
	    slice->svmstring != NULL)
		slice_to_use = xstrdup(slice->svmstring);
	else
		slice_to_use = xstrdup(slice->slice);
...
AppDoMountsAndSwap(UpgOs_t *slices)
{
	...
	if (!slice)
		return (FAILURE);
	err = mount_and_add_swap(slice->slice, NULL);
	...
	char **main_label, char **detail_label)
{
	...
	case VAL_NEWPKG_SPACE:
		...
		break;
	case VAL_EXEC_LOCAL_PKGADD:
		main_str = LABEL_UPG_VAL_EXEC_LOCAL_PKGADD;
		break;
	case VAL_EXEC_VIRTUAL_PKGADD:
		main_str = LABEL_UPG_VAL_EXEC_VIRTUAL_PKGADD;
		break;
	case VAL_EXEC_PKGRM:
	...
	case VAL_EXEC_REMOVEF:
		...
		break;
	case VAL_EXEC_LOCAL_SPOOL:
		main_str = LABEL_UPG_VAL_EXEC_LOCAL_SPOOL;
		break;
	case VAL_EXEC_VIRTUAL_SPOOL:
		main_str = LABEL_UPG_VAL_EXEC_VIRTUAL_SPOOL;
		break;
	 ...
	 * the main string looks like:
	 *	"main str..."
	 * If there is a non-global zone, then
	 * the main string looks like:
	 *	"main str:<zone> "
	 */
	if (val_progress->valp_detail) {

		/* indicate local (non-global) zones for certain stages */
		if ((val_progress->valp_stage == VAL_EXEC_LOCAL_PKGADD ||
		    val_progress->valp_stage == VAL_EXEC_VIRTUAL_PKGADD ||
		    val_progress->valp_stage == VAL_EXEC_PKGRM ||
		    val_progress->valp_stage == VAL_EXEC_REMOVEF ||
		    val_progress->valp_stage == VAL_EXEC_RMPATCH) &&
		    val_progress->valp_zonename != NULL) {

			write_debug(APP_DEBUG_L1,
			    "Update progress: Zonelist = %s",
			    val_progress->valp_zonename);


			(void) snprintf(buf, PATH_MAX, "%s:<%s>",
			    main_str, val_progress->valp_zonename);
		} else {
			(void) snprintf(buf, PATH_MAX, "%s:", main_str);
		}
...
/*
 * Function: stringBadZoneList
 * Description:
 *	Takes a list of zone names and lists them in a formatted output
 *	buffer inserting a left margin, delimiters, line feeds,
 *	and supplies an ellipsis if not all zones can fit on the screen.
 *
 *	Dimensions are taken from available screen space on the ttinstall
 *	failed slice curses screen.
 *
 * Scope:	PRIVATE
 * Parameters:
 *	label:
 *		text string to precede zone list
 *
 * Return:	ptr to message plus zone list allocated on heap
 * Globals:	none
 * Notes:	allocates buffer big enough to hold
 *		as many characters as can fit on a screen with the specified
 *		dimensions
 *		frees zone list from heap
 */
static char *
stringBadZoneList(char *label)
{
#define	ZROWLEN 50	/* length of zone list output rows */
#define	ZMAXROWS 11	/* number of zone list rows to fit on screen */
#define	ZMARGIN	"   "	/* left margin */
#define	ZDELIMITER ","	/* separates zone names */
	int		nrows = 0;		/* counts rows */
	int		lendelimiter		/* zone delimiter length */
				= sizeof (ZDELIMITER) - 1;
	int		truncate = 0;		/* zone list truncated? */
	char		*pcurrow;		/* start of current row */
	StringList	*pz;			/* current zone name */
	char		*msgbuf;		/* message to return */

	/* allocate message buffer from heap */
	msgbuf = xmalloc(ZMAXROWS *
		    (ZROWLEN + 20) + /* length of one row plus padding */
		    strlen(label));

	(void) strcpy(msgbuf, label);	/* place label in front */
	/*
	 * append the list of zones (formatted for screen output)
	 * to the zone failure message
	 */
	pcurrow = msgbuf + strlen(msgbuf);	/* point to 1st row */
	(void) strcpy(pcurrow, ZMARGIN);	/* copy margin into 1st row */

	for (pz = pBadZoneList; pz != NULL; pz = pz->next) {
		/* for each zone */

		if (pz->next == NULL)
			lendelimiter = 0; /* EOT */
		/* will another zone fit into row? */
		if (ZROWLEN + strlen(ZMARGIN) <
		    strlen(pcurrow) +
		    strlen(pz->string_ptr) + /* zone name */
		    lendelimiter) {

			/* finish the current row */
			(void) strcat(pcurrow, "\n");
			/* reset line pointer */
			pcurrow += strlen(pcurrow);
			/* start new line */
			(void) strcpy(pcurrow, ZMARGIN);
			if (++nrows >= ZMAXROWS) {	/* over row limit? */
				truncate = 1;
				break;
			}
		}
		/* append current zone to line buffer */
		(void) strncat(pcurrow, pz->string_ptr, ZROWLEN);
		if (lendelimiter)
			(void) strcat(pcurrow, ZDELIMITER);
	}	/* next zone */
	if (truncate) /* out of lines - tack on ellipsis */
		(void) strcat(msgbuf, "...");
	StringListFree(pBadZoneList); /* return bad zone list to heap */
	return (msgbuf);
}
 ...
 * Notes:
 *	for zones error,
 *	- expects bad zone list in static
 *	- returns bad zone list to heap
 */
char *
AppGetUpgradeErrorStr(int error_code, char *release, char *slice)
{
	...
	char *msg;
	int slice_str_len;
	char *allocmsg = NULL;	/* if set, msg buffer alloc'd from heap */
	...
	case ERR_MOUNT_FAIL:
		...
		break;
	case ERR_ZONE_MOUNT_FAIL:
		msg = APP_MSG_ZONE_MOUNT_FAILED;
		break;
	...
	case ERR_ZONE_FAIL:
		/*
		 * for non-upgradeable zones, make a formatted list of
		 * zone names and append them to the standard message
		 *
		 * expects list of bad zones in static StringList
		 */

		msg = (allocmsg = stringBadZoneList(APP_MSG_ZONE_FAILED));
		/* save heap address for freeing later */
	...
	slice_str_len = strlen(APP_ER_SLICE_CANT_UPGRADE) +
	    strlen(msg) +
	    strlen(release) +
	    strlen(slice) +
	    1;
	str = (char *)xmalloc(slice_str_len);
	(void) snprintf(str, slice_str_len, APP_ER_SLICE_CANT_UPGRADE,
		release,
		slice,
		msg);

	if (allocmsg != NULL) /* free message buffer if allocated from heap */
		free(allocmsg);
