...
static int _DsrSLEntryIsExempt(char *entryDevicePath);
	...
	LL_WALK(slhandle, slcurrent, slentry, err) {
		...
		if (slentry->InVFSTab) {
			...
			if (DsrIsDeviceCoResident(fs_space,
				...
				/* 
				 * If the entry is the default Swap
				 * partition. Set it to fixed
				 */
				if (_DsrSLEntryIsExempt(slentry->SliceName) == TRUE) {
...
_DsrSLAutoLayoutDiskSetup(TList slhandle)
{
	...
	LL_WALK(slhandle, slcurrent, slentry, err) {
		 ...
		 */
			...
			/*
			 * Collapsed entries are just available space,
			 * so ignore it just like we do non-vfstab
			 * available entries.
			 */
...
DsrSLCreate(TList *slice_list, TLLData list_data, FSspace **fs_space)
{
	...
	/*
	 * deselect all disks without Solaris partitions to prevent
	 * erroneous attempts to use them for the upgrade
	 */
	write_debug(APP_DEBUG_L1, "Checking need to deselect disks");
	if (IsIsa("i386"))
		DiskDeselectNonSolaris(CFG_CURRENT);
	...
	for (i = 0; fs_space && fs_space[i]; i++) {
		 ...
		 * the exemptSwapDisk. if it is,
		 * make it fixed.
		 */
		if (_DsrSLEntryIsExempt(fs_space[i]->fsp_fsi->fsi_device) == TRUE) {
			SliceState = SLFixed;
		}
		/*
		 ...
		 * parent file system
		 */
		else if (fs_space[i]->fsp_flags & FS_IGNORE_ENTRY) {
	...
	WALK_DISK_LIST(dp) {
		/*
		 * all disks with Solaris partitions have been selected,
		 * so all the SliceobjFindUse calls will work on them.
		 * autolayout also requires any disks that we're using
		 * to be selected.  NOTE: all disks without Solaris
		 * partitions have been deselected.
		 */
 ...
 * Globals:	none
 * Notes:
 *	The default values here are very important.
 *	Because there is no "fixed" layout_constraint keyword value in
 *	pfinstall, the only way a user can specify "fixed" is by having
 *	the file systems default to fixed here when they use the
...
DsrSLEntrySetDefaults(TSLEntry *slentry)
{
	...
	if (slentry->InVFSTab) {
		if (_DsrSLEntryIsExempt(slentry->SliceName) == TRUE) {
		    	/* 
			 * Entry is the exempt Swap
		     	 * partition. Set it to fixed
			 */
		    	slentry->State = SLFixed;
		} else if (SL_SLICE_IS_COLLAPSED(slentry)) {
	...
	Disk_t **orig_dp, int *orig_slice)
{
	...
	/* Initialize orig_dp and orig_slice */
...
/*
 * Function:	_DsrSLEntryIsExempt
 * Description:
 *	Determine if the exemptSwapDisk is the current entry
 * Scope:	PUBLIC
 * Parameters:
 *	the current slice entry's device path
 *
 * Return:	TRUE
 *		FALSE
 * Globals:
 ...
 */
int
_DsrSLEntryIsExempt(char *entryDevicePath)
{
	char *exemptSwapDisk;
	char buf[MAXPATHLEN+1];
	
	if (entryDevicePath == NULL) {
	    	return (FALSE);
	}

	if (is_slice_name(entryDevicePath)) {
	    	sprintf(buf, "/dev/dsk/%s", entryDevicePath);
	} else {
	    	strcpy(buf, entryDevicePath);
	}
	...
	if (strcmp(buf, exemptSwapDisk) == 0){
	    	return (TRUE);
	}
