...
#pragma ident	"@(#)pkgpatch.c	1.3	06/02/27 SMI"
...
get_fs_bo_dir(char *ir, char *patch, char *pkginst, char **bo)
{
	...
	*bo = NULL;
	...
	if (snprintf(pkginfo_file, PATH_MAX, "%s/var/sadm/pkg/%s/pkginfo",
			ir, pkginst) >= PATH_MAX)
		return (-1);
	...
	if (snprintf(patchdir, PATH_MAX, "%s/var/sadm/pkg/%s/save/%s",
...
put_patch_info_old(char *ir, genericdb *gdb)
{
	...
	if (snprintf(pkgdir, PATH_MAX, "%s/var/sadm/pkg", ir) >= PATH_MAX)
		return (-1);
