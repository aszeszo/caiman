...
class InnerWindow(object):
    ...
    def _init_win(self, parent):
        ...
        if self.is_pad:
            terminalui.LOGGER.debug("lines=%d, columns=%d, y_loc=%d, x_loc=%d",
    ...
    def activate_object(self, index=0, loop=False, jump=False):
        ...
        if not isinstance(index, int):
            ...
            index = min(len(self.objects) - 1, max(0, index))
    ...
    def _init_key_dict(self):
        ...
        self.key_dict[curses.KEY_NPAGE] = functools.partial(self.on_page, 1)
