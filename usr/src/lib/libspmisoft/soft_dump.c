...
#pragma ident	"@(#)soft_dump.c	1.10	07/11/09 SMI"
...
/* ******************************************************************** */
boolean_t
dump_media_mod(FILE *fp, Module *mod)
{
	if (fp == NULL)
		return (B_FALSE);
...
dump_modinfo(Modinfo * mi)
{
	...
	if (mi->m_next_patch != NULL) {
		(void) fprintf(dfp, "%sPATCH INSTANCES:\n",
		    indent[dump_level]);
		dump_level++;
		mip = mi;
...
dump_modinfo_fields(Modinfo * mi)
{
	...
	if (mi->m_pkg_hist) {
		...
		if (mi->m_pkg_hist->prod_rm_list)
			(void) fprintf(dfp, "%sprod_rm_list = %s\n",
					indent[dump_level],
					mi->m_pkg_hist->prod_rm_list);
 ...
 *
 * Parameters:
 *	geo	- pointer to geo structure to be dumped
 * Return:
 *	none
 * Status:
 *	semi-private (internal library use only)
...
dump_geo(Geo *geo)
{
	...
	if (geo->g_geo)
		(void) fprintf(dfp,
				"%sg_geo = %s\n",
				indent[dump_level], geo->g_geo);
	if (geo->g_name)
		(void) fprintf(dfp,
				"%sg_name = %s\n",
				indent[dump_level], geo->g_name);
	if (geo->g_selected)
		(void) fprintf(dfp,
				"%sg_selected = %d\n",
				indent[dump_level], geo->g_selected);
	if (geo->g_locales) {
		(void) fprintf(dfp, "%sg_locales =", indent[dump_level]);
		for (str = geo->g_locales; str; str = str->next)
			(void) fprintf(dfp, " %s", str->string_ptr);
		(void) fprintf(dfp, "\n");
	}
}
...
dump_mediainfo(Media * med)
{
	...
	if (med->med_zonename != NULL) {
		(void) fprintf(dfp, "%smed_zonename = %s\n",
		    indent[dump_level], med->med_zonename);
	}
