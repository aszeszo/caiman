...
#pragma ident	"@(#)soft_module.c	1.8	07/11/12 SMI"
...
static Module	*find_pkg_module(Module *, char *);
 ...
 * Return:
 *	SUCCESS	    -	if successful ERR_NOMEDIA -
 *			if mod type is not a MEDIA
 ...
 * Return:
 *	ERR_INVALIDTYPE	- module type passed in was NULL, MEDIA, or PRODUCT
...
swi_mark_module(Module * modp, ModStatus status)
{
	...
	if (modp->info.mod->m_status != REQUIRED) {
		...
		if (prod && prod->info.prod->p_packages != NULL) {
			...
			if (np != NULL)
				(void) update_selected_arch(np, (caddr_t)prod);
 ...
 *		or some were REQUIRED, then return PARTIALLY_SELECTED.
 * Parameters:
...
swi_mod_status(Module * mod)
{
	...
	for (mp = mod->sub; mp; mp = mp->next) {
		...
		if (mp->info.mod->m_status == SELECTED ||
			...
			m++;
		if (mp->info.mod->m_status == PARTIALLY_SELECTED)
			p++;
 ...
 *	refcnt is decremented). If 'mod' is PARTIALLY_SELECTED, and something
 *	under 'mod' is SELECTED (rather than REQUIRED) then the sub-tree is
 *	UNSELECTED.  Otherwise, everything is SELECTED. Used only by
 *	ttinstall, mainly for historical reasons.
 * Parameters:
...
#define	mod_info ((Modinfo *) mod->info.mod)
	...
	if (mod_info->m_status != REQUIRED && mod_info->m_status != LOADED) {
		if (mod_status(mod) == PARTIALLY_SELECTED)
...
/*
 * Function:	get_pkg_module
 * Description:	Retrieves the package Module pointer from the
 *		current product based on a package name.
 * Arguments:	pkgname - The package name
 * Returns:	The Modinfo pointer for the package, or NULL if it doesn't exist
 */
Module *
get_pkg_module(char *pkgname)
{
	Module	*mod;
	Node	*node;

	mod = get_head(get_default_metacluster());
	return (find_pkg_module(mod, pkgname));

}

/*
 * Function:	find_pkg_module
 * Description:	Recursively finds the package Module pointer from the
 *		current product based on a package name.
 * Arguments:	pkgname - The package name
 *		head - The module at which to start looking.
 * Returns:	The Modinfo pointer for the package, or NULL if it doesn't exist
 */
Module *
find_pkg_module(Module *head, char *pkgname) {
	Module	*childptr;
	Module	*siblingptr;

	/* are we the package module that is being looked for? */
	if (head->info.mod->m_pkgid &&
	    (strcmp(head->info.mod->m_pkgid, pkgname) == 0) &&
	    (head->type == PACKAGE)) {
		return (head);
	}
	...
	/* ask our children */
	if (head->sub) {
		if ((childptr = find_pkg_module(head->sub, pkgname)) != NULL) {
			return (childptr);
		}
	}
	...
	/* ask our sibling */
	if (get_next(head)) {
		return (find_pkg_module(get_next(head), pkgname));
	}
 ...
 * Description: Function to determine if package was selected for install,
 *              based upon status in Modinfo.  Function is
 *              called for each package in the transfer list.
 *              Returns 0 is not installed, 1 if installed.
 */
int
pkg_installed(char *pkgname)
{
	Module *mod;

	mod = get_pkg_module(pkgname);
	if (mod == NULL) {
		return (0);
	}
	if (mod->info.mod->m_status == INSTALL_SUCCESS) {
	    return (1);
	}
...
/*
 * Function:	mark_pkg_required
 * Description: 	Function to mark a package module as being required,
 *		based on a package name.
 * Arguments:	pkgname - The package name
 * Returns:	none.
 */
int
mark_pkg_required(char *pkgname)
{
	Module	*mod;
...
#define	mod_info	((Modinfo *) mod->info.mod)
	...
	for (mod = first; mod != NULL; mod = mod->next) {
		 ...
		 */
		if ((mod->type == PACKAGE) && mod_info->m_l10n && prod)
			sync_l10n(prod);
	}
#undef mod_info
}
