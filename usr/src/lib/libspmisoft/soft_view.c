...
#pragma ident	"@(#)soft_view.c	1.6	07/11/12 SMI"
...
static int	change_view_geo(Node *, caddr_t);
...
load_default_view(Module * prod)
{
	...
	walklist(prod->info.prod->p_view_geo, change_view_geo, NULL);
...
load_view(Module * prod, Module *media)
{
	...
	if (p == NULL) {
		...
		walklist(p->p_view_geo, change_view_geo, NULL);
...
clear_all_view(Module *prod)
{
	...
	for (mod = prod->info.prod->p_geo; mod != NULL; mod = mod->next)
		mod->info.geo->g_selected = 0;
...
clear_package_view(Node * np, caddr_t data)
{
	...
	for (mi = (Modinfo *)(np->data); mi != NULL; mi = next_inst(mi)) {
		 ...
		 * and IS_VIRTUAL_PKG
		 */
		mi->m_flags &= (PART_OF_CLUSTER|IS_VIRTUAL_PKG);
		if (mi->m_instdir != NULL) {
			free(mi->m_instdir);
			mi->m_instdir = NULL;
		}
...
/*ARGSUSED1*/
static int
change_view_geo(Node *np, caddr_t data)
{
	View	*view;

	view = (View *)np->data;
	view->v_info.v_geo->g_selected = *view->v_status_ptr;
	return (SUCCESS);
}
...
save_current_view(Module * prod)
{
	...
	/* Save the view of the geos */
	if (p->p_view_geo == NULL)
		p->p_view_geo = getlist();
	for (mp = p->p_geo; mp != NULL; mp = mp->next) {
		if ((n = findnode(p->p_view_geo, mp->info.geo->g_geo))
								== NULL) {
			v = (View *)xcalloc(sizeof (View));
			v->v_type = GEO;
			v->v_info.v_geo = mp->info.geo;
			v->v_status_ptr = &v->v_status;
			*v->v_status_ptr = mp->info.geo->g_selected;
			n = getnode();
			n->key = xstrdup(mp->info.geo->g_geo);
			n->data = v;
			n->delproc = &free_np_view;
			(void) addnode(p->p_view_geo, n);
		} else {
			v = (View *)n->data;
			*v->v_status_ptr = mp->info.geo->g_selected;
		}
