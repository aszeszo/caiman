...
static int 	g_upgrade_clients = 0;
...
static char	*_translate_sun4u1_arch(Module *, char *);
...
upgrading_clients()
{
	return (g_upgrade_clients);
}

void
set_upgrade_clients(int upg)
{
	g_upgrade_clients = upg;
}
...
set_initial_selected_arches(Module *mod, Module *mod_upg_to)
{
	...
	for (ap = mod->sub->info.prod->p_arches; ap != NULL; ap = ap->a_next) {
		if (ap->a_loaded) {
			/*
			 * HACK: E10k machines were sun4u1 in 2.6, and sun4u in
			 * subsequent releases.  This routine is trying to
			 * select in the release being installed the
			 * architectures installed on the machine being
			 * upgraded.  We need to translate sun4u1 to sun4u if
			 * we see it.  This can go away when upgrades from 2.6
			 * are no longer supported.
			 */
			arch = _translate_sun4u1_arch(mod, ap->a_arch);

			(void) select_arch(u_newproductmod, arch);
			if (mod_upg_to && mod_upg_to != mod &&
			    mod_upg_to->sub) {
				add_arch(mod_upg_to->sub, arch);
				select_arch(mod_upg_to->sub, arch);
			}
		}
	}
}
...
/*
 * Translate `sun4u1' to `sun4u' if we're coming from a system installed
 * with 2.6 or prior.  See 4325840.  This routine can go away when upgrades
 * from 2.6 are no longer supported.
 */
static char *
_translate_sun4u1_arch(Module *mod, char *arch)
{
	char *srcrev;
	int srcrev_len;
	int ret;
	...
	if (!streq(arch, "sparc.sun4u1")) {
		/* Nope - not sun4u1 */
		return (arch);
	}

	srcrev_len = strlen(mod->sub->info.prod->p_name) +
	    strlen(mod->sub->info.prod->p_version) + 2;
	srcrev = xmalloc(srcrev_len);
	(void) snprintf(srcrev, srcrev_len, "%s_%s",
	    mod->sub->info.prod->p_name, mod->sub->info.prod->p_version);

	if ((ret = prod_vcmp(srcrev, "Solaris_2.6")) == V_EQUAL_TO ||
	    ret == V_LESS_THEN) {
		/* sun4u1 coming from 2.6 or prior */
		arch = "sparc.sun4u";
	}
...
initialize_upgrade(void)
{
	...
	if (!pkg_hist_path)
		(void) snprintf(path, sizeof (path), "%s/.pkghistory",
	...
	if (!pkg_hist_path)
		(void) snprintf(path, sizeof (path), "%s/.clusterhistory",
		    u_newmedia->sub->info.prod->p_pkgdir);
	else
		(void) snprintf(path, sizeof (path), "%s/.clusterhistory",
		    pkg_hist_path);
...
setup_for_local_upgrade(void)
{
	...
	/* find all nonglobal zones and mark them to be upgraded */
	for (mod = get_media_head(); mod != NULL; mod = mod->next) {
		if (mod->info.media->med_type != INSTALLED)
			continue;
		if (strcmp(mod->info.media->med_dir, "/") == 0)
			continue;  /* it's the global zone localmedia */

		/* Make sure it's a nonglobal zone, and mark it for upgrade. */
	...
	(void) snprintf(media_version, MAXPATHLEN, "%s_%s",
	    u_newproduct->p_name, u_newproduct->p_version);
	(void) snprintf(media_service, MAXPATHLEN, "/export/%s", media_version);
	...
	if (strcmp(media_version, svc_to_be_upg->info.media->med_volume) == 0) {
		...
		(void) strcpy(new_svc_mod->sub->info.prod->p_pkgdir,
		    media_service);
		(void) strcat(new_svc_mod->sub->info.prod->p_pkgdir,
		    "/var/sadm");
...
setup_for_nonnative_upgrade(StringList *client_list)
{
	...
	if (shared_service) {
		...
		(void) snprintf(media_version, sizeof (media_version),
		    "%s_%s", u_newproduct->p_name, u_newproduct->p_version);
		(void) snprintf(media_service, sizeof (media_service),
		    "/export/%s", media_version);
