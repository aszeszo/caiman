...
#pragma ident	"@(#)soft_find_modified.c	1.22	07/11/09 SMI"
...
/* i18n'd messages */
#define	CANT_PROCESS	gettext("Can't process contents entry %s\n")
...
static List	*read_db(genericdb *);
static int	checkmap(char *, Product *);
static void	canonize_priv(char *);
static void	mappath_priv(int, char *);
static void	basepath_priv(char *, char *);

static int	checkdb(genericdb *, Product *);
...
static int	gpkgmap_priv(struct cfent *, FILE *);
...
static struct filediff *cverify_priv(char *path, struct cinfo *cinfo,
    struct filediff *statp);
static struct filediff *averify_priv(char *ftype, char *path,
    struct ainfo *ainfo);
...
/* In child process within non-global zone */
extern boolean_t is_child_zone_context;
...
find_modified(Module *mod)
{
	...
	if (pkgdb_supported() && genericdb_exists(install_root)) {
	    if ((gdb = genericdb_open(install_root, 0400,
		0, NULL, &gdbe)) != NULL) {
		/* we have a new database, so check it instead */
		(void) checkdb(gdb, mod->sub->info.prod);
		(void) genericdb_close(gdb);
		gdb = NULL;
	    }
	} else {
		/* revert to the old  contents file database */
		(void) snprintf(mapfile, sizeof (mapfile),
...
static genericdb *upg_scan_db = NULL;
static List *upg_scan_list = NULL;
static int upg_scan_entry_in_buf = 0;

/*
 * name:  file_will_be_upgraded_db
 *
 * description:  determines if a file is to be upgraded during the
 *	upgrade process.
 *
 * parameters:
 ...
 *		or:	       /usr/bin/cat
 *	contents_db - the path of the contents db, relative to
 *		the rootdir.
 *		example:  /export/root/client-1/var/sadm/install
 *		or:       /var/sadm/install
 *	reset - if non-zero, this is a new contents DB, or a re-open
 *		the current contents DB.  The "contents_db" argument
 *		will never change from one call of the function to the
 *		next unless "reset" is non-zero.
 *
 * return:
 *
 *	0 - file will NOT be replaced or deleted during an upgrade.
 *	This is significant during a DSR upgrade, because for files
 *	that aren't going to be replaced, we need to save a copy off
 *	to some temporary location while we re-format the disk to
 *	house the new upgrade.
 *
 *	> 0 - file WILL be replaced, or will be deleted as part of the
 *	upgrade.  This is significant during a DSR upgrade because
 *	this means we can ignore the file, not make a backup copy,
 *	since we know that its contents are irrelevant to the outcome
 *	of the upgrade process.
 *
 *	< 0 - fatal error determine upgrade status of file.
 *
 * Side effects:
 *
 *	The temporary static 'baseroot' buffer is modified
 *
 *	If reset is != 0, upg_scan_db is initialized with the
 *	newly-opened database handle
 *
 *	If reset is != 0, upg_scan_list is populated with
 *	the contents read from the database.
 */
int
file_will_be_upgraded_db(char *inpath, char *contents_db, int reset)
{
	...
	genericdb_Error open_db_err;

	if (inpath == NULL || contents_db == NULL) {
		(void) printf(gettext("file_will_be_upgraded: null input.\n"));
		return (-2);
	}
	...
	if (reset) {
		/*
		 *  Truncate the "/var/sadm/install" part of
		 *  the contents file path to get the root (relative
		 *  to get_rootdir()) of the environment that corresponds
		 *  to the contents file.
		 */
		(void) strcpy(mapworkarea, contents_db);
		ptr = strstr(mapworkarea, "/var/sadm/install");
		if (ptr == NULL) {
			(void) printf(
			    gettext("file_will_be_upgraded: Bad contents "
				"db.\n"));
			return (-2);
		}
		*ptr = '\0';
		if (strlen(mapworkarea) == 0)
			(void) strcpy(mapworkarea, "/");

		/*
		 *  Find the product structure whose p_rootdir value
		 *  matches the root of the specified contents db.
		 */
		...
		if (medmod == NULL) {
			(void) printf(gettext(
			    "file_will_be_upgraded: Bad contents db.\n"));
			return (-2);
		}
		...
		if (baseroot[n - 1] == '/')
			baseroot[n - 1] = '\0';
		(void) snprintf(mapworkarea, sizeof (mapworkarea),
			"%s/var/sadm/install", baseroot);

		if (!streq(curmapfile, mapworkarea)) {
			if (upg_scan_list != NULL) {
				dellist(&upg_scan_list);
			}
			if ((upg_scan_db =
			    genericdb_open(baseroot, 0400, 0,
				NULL, &open_db_err)) == NULL) {
				(void) printf(
					gettext("Can't open contents "
					    "db: %s"),
					    genericdb_errstr(open_db_err));
				return (-2);
			}

			/* read entire db into a hashtable */
			if ((upg_scan_list = read_db(upg_scan_db)) == NULL) {
				(void) printf(
					gettext("Can't open contents "
					    "db: %s"),
					    genericdb_errstr(open_db_err));
			...
			/* close db */
			(void) genericdb_close(upg_scan_db);
			upg_scan_db = NULL;
			...
			if (streq(medmod->sub->info.prod->p_rootdir, "/"))
				(void) strcpy(curprodroot, "/");
			else {
				(void) strcpy(curprodroot,
				    medmod->sub->info.prod->p_rootdir);
				(void) strcat(curprodroot, "/");
			}
	...
	if (ptr == NULL || ptr != inpath) {
		...
		if (streq(inpath, medmod->sub->info.prod->p_rootdir))
			...
			(void) printf(gettext(
				"file_will_be_upgraded: input path not "
	...
	if ((ent = findnode(upg_scan_list, inpath)) == NULL) {
	    /* isn't in contents db */
	    return (0);
	}

	/* inpath is in the contents file */
	new_media_mod = get_new_media_head();
	for (pp = ((struct cfent *)(ent->data))->pinfo; pp != NULL;
	    pp = pp->next) {
		...
		if (mi == NULL)
			continue;
		/*
		 * Is this package going to be on the upgraded system?
		 */
		...
		if (mnew == NULL) {
			if (mi->m_pkg_hist &&
				 ...
				 * check for the file in the replacing
				 * package
				 */
				ptr = mi->m_pkg_hist->replaced_by;
				while ((pkg_name = split_name(&ptr))
				    != NULL) {
			...
			} else if (mi->m_action == TO_BE_REMOVED) {
				/*
				 * This package is to be removed,
				 * so we tell the caller the file
				 * will be upgraded (actually it'll
				 * go away with pkgrm)
				 */
				return (1);
			}
		...
		if (mnew) {
				/*
				 * If the file has been modified, need
				 * to keep it
				 */
 ...
 *	return FALSE.
 *	*Also* return TRUE if the file is part of a package that is
 *	TO_BE_REMOVED, since:
 *	a.	Why waste cycles backing it up only to have it pkgrm'd
 *	b.	Space calculation didn't account for it, so DSR restore
 *		may fail
 *
 * parameters:
 *	inpath - the file being examined for whether it will be
...
file_will_be_upgraded(char *inpath, char *contents_file, int reset)
{
	...
	if (inpath == NULL || contents_file == NULL) {
		(void) printf("file_will_be_upgraded: null input.\n");
		return (-2);
	}
	...
	if (reset) {
		...
		if (ptr == NULL) {
			(void) printf(
			    "file_will_be_upgraded: Bad contents file.\n");
			return (-2);
		}
		*ptr = '\0';
		if (strlen(mapworkarea) == 0)
	...
	if (ptr == NULL || ptr != inpath) {
		...
		if (streq(inpath, medmod->sub->info.prod->p_rootdir))
			...
			(void) printf("file_will_be_upgraded: input path not"
			    " relative to contents file root.\n");
			return (-2);
		}
	} else
	...
	while (n = get_next_contents_entry(fp, &entry)) {
		...
		if (install_root) {
			...
			(void) snprintf(temppath, sizeof (temppath), "%s/%s",
			    install_root, entry.path);
			entry.path = temppath;
		}

		if (prod->p_inheritedDirs != NULL &&
		    *prod->p_inheritedDirs != NULL) {
			canonize_priv(entry.path);
			if (z_path_is_inherited(entry.path, entry.ftype,
			    prod->p_rootdir)) {
				if (install_root) {
		...
		if (((statp = ckentry(0, &entry))) != NULL) {
			if ((int)statp == -1) {
				(void) printf(CANT_PROCESS, entry.path);
				return (-1);
			} else {
 ...
 * description:
 *
 *	For each entry in the contents database, determine whether the
 *	file on the system has changed from what was initially installed.
 *	Attributes checked are: owner, group, permissions, contents
 *	and timestamps on the file.  If something has changed,
 *	add it to the list of files that have changed so the
 *	files can be appropriately massaged after the upgrade
 *	(by changing its owners back, or modifying other attributes).
 *
 * parameters:
 *	gdb -	Handle to currently-open contents database.
 *
 *	prod - 	Current product being upgraded.
 *
 * return:
 *	0 - All checking completed with no errors
 *
 *	-1 - error checking attributes of one or more files
 *	This error can be ignored.  It is not fatal.
 *
 * Side effects:
 *
 *	Global eptnum and eptlist are populated with contents
 *	from database.
 *
 *	p_modfile_list list within the prod structure is populated
 *	with list of modified files found during this checkdb invocation.
 *
 */
static int
checkdb(genericdb *gdb, Product *prod)
{
	int	n;
	char 	*save_path = NULL;
	char	temppath[MAXPATHLEN];
	...
	if (get_pkg_db(NULL, NULL, gdb) != 0) {
		(void) printf(gettext("Can't access contents database"));
		return (-1);
	}
...
struct cfent	*ept;
{
	...
	canonize_priv(ept->path);
	if (strchr("sl", ept->ftype)) {
		if (!RELATIVE(ept->ainfo.local)) {
			if (envflag) {
				mappath_priv(2, ept->ainfo.local);
				basepath_priv(ept->ainfo.local, basedir);
			}
			canonize_priv(ept->ainfo.local);
		}
	}
	...
	if (!strchr("in", ept->ftype)) {
		/* validate attributes */
		statp = averify_priv(&ept->ftype, ept->path, &ept->ainfo);
		if (statp == (struct filediff *)-1) {
			return (statp);
		}
	}
	if (statp && statp->diff_flags & DIFF_MISSING) {
		...
		missp = (struct missing_file *)xcalloc(
		    sizeof (struct missing_file) +
		    strlen(lastmissing) - bump + 1);
	...
	if (strchr("fev", ept->ftype)) {
		/* validate contents */
		statp = cverify_priv(ept->path, &ept->cinfo, statp);
		...
		if (n == 0) {
			...
			if (c == '=') {
				...
				if (getstr(fpin, NULL, MAXPATHLEN, mylocal))
					RETERROR("unable to read local/link "
					    "path")
			 ...
			 */
			if (fseek(fpin, pos, 0)) {
				srcherrstr = gettext("failure attempting "
				    "fseek");
				return (-1);
			}
	...
	if (rdpath < 2) {
		...
		switch (c = eatwhite(fpin)) {
			...
			if (getstr(fpin, "=", MAXPATHLEN, mypath))
				RETERROR(gettext("unable to read pathname "
				    "field"))
			ept->path = mypath;
...
#endif
			RETERROR(gettext("unable to read major/minor device "
			    "numbers"))
	}
...
#define	ERR_CHDIR	gettext("unable to change current working " \
				"directory to <%s>")
	...
	switch (c = eatwhite(fp)) {
		...
		if (ept->ftype == 'i') {
			...
			if (!getnum(fp, 10, (long *)&ept->cinfo.size,
			    ...
			    BADCONT))) {
				errstr =
				    gettext("unable to read content info");
				goto error;
			}
		}
		...
		if (c == '=') {
			/* local path */
			if (getstr(fp, NULL, MAXPATHLEN, mylocal)) {
				errstr = (strchr("sl", ept->ftype) ? gettext(
			...
			if ((c != EOF) && (c != '\n'))
				(void) getend(fp);
			errstr =
			    gettext("missing or invalid link specification");
			return (-1);
		} else
			(void) ungetc(c, fp);
		break;
...
add_statp_entry(struct filediff *statp, Modinfo *mi)
{
	...
	if ((mi->m_pkg_hist == NULL) ||
		...
		pkg_entry = (struct pkg_info *)xmalloc(
		    sizeof (struct pkg_info));
...
check_statp_chain()
{
	...
	for (tbl = tbl_head; tbl != NULL; tbl = tbl->next) {
		...
		gen_pkgmap_path(mapfile, mod->sub->info.prod->p_pkgdir, m_new);
		if ((fp = fopen(mapfile, "r")) == NULL)
			continue;

		while (ndx = gpkgmap_priv(&pentry, fp)) {
		...
		if (real_modified_list == NULL) {
			...
			real_modified_list_tail->diff_next =
...
chain_match_component(struct cfent *pentry, Modinfo *m_new)
{
	...
	(void) snprintf(path_string, sizeof (path_string), "%s/%s",
	    m_new->m_instdir != NULL ?  m_new->m_instdir : m_new->m_basedir,
	    pentry->path);
 ...
 * ************************************************************************
 * **
 * **   The next five functions are used to free malloc'ed space.
 * **
 * ************************************************************************
 */
static void
...
free_statp_chain()
{
	...
	for (ptr = statp_chain_head; ptr != NULL; ptr = next) {
		/*
		 * NOTE:  The following command and block of code were
		 * added as part of the fix to 1195508 (see comments
		 * on file libsw/find_modified.c, which is the ancestor
		 * of this file).  At this time (late in the development
		 * of 2.6), I am reluctant to back out that earlier fix
		 * without extensive testing, but it appears to be wrong:
		 * the only files left on the statp chain at this point
		 * should be those for which NO replacement file was found
		 * in any package, so it would seem to always be safe to
		 * delete the statp entry and therefore leave the file
		 * alone during the upgrade.  But since that's how the
		 * USED to work, and was changed to work like this, and
		 * it's very late in the release cycle, I am making a more
		 * limited change than backing it all out.  I am adding
		 * the check for editable files to the conditions for
		 * adding the file to the real_modified_list.  It seems
		 * clear that editable files should not be renamed,
		 * because the package that's being removed may want to
		 * edit them during removal.  That's exactly what's
		 * happening in bug 1266409.
		 * Here's the comment from the earlier fix:
		 *
...
add_new_fs_dev(dev_t dev)
{
	...
	for (ptr = fs_dev_head; ptr != NULL; ptr = ptr->next_fs_dev)
		if (ptr->fs_dev == dev)
			return;
	ptr = (struct fs_dev_chain *)xcalloc((size_t)
	    sizeof (struct fs_dev_chain));
	ptr->fs_dev = dev;
	...
	if (strncmp(inpath, spooled_pkg_path, strlen(spooled_pkg_path)) == 0) {
		for (mod = get_media_head(); mod != NULL; mod = mod->next) {
			...
			if (mod->sub->info.prod->p_name == NULL ||
				...
				continue;
			(void) snprintf(svc_name, sizeof (svc_name), "%s_%s/",
...
/*
 * Name:	read_db
 * Desc:	Reads entire DB into a hashtable for later speedy lookup
 *	gdb - The database to read
 * Retn:	A hashtable representing the entire package DB, or NULL
 *	on error or of no database was found
 */
static List
*read_db(genericdb *gdb)
{
	List *newlist;
	int n;
	Node *node;
	...
	/* add each entry to hashtable */
	for (n = 0; n < eptnum; n++) {
		node = getnode();
		node->key = xstrdup(eptlist[n]->path);
		node->data = eptlist[n];
		(void) addnode(newlist, node);
	}
...
/*
 * Name:	write_real_modified_list_to_pipe
 * write the list of modified files
 * Desc:	Reads entire DB into a hashtable for later speedy lookup
 *	gdb - The database to read
 * Retn:	A hashtable representing the entire package DB, or NULL
 *	on error or of no database was found
 */
int
write_real_modified_list_to_pipe(FILE *fp)
{
	return (write_filediff_owning_pkg_to_pipe(fp, real_modified_list));
}

int
read_real_modified_list_from_pipe(FILE *fp, Module *mod)
{
	return (read_filediff_owning_pkg_from_pipe(fp, mod));
}
