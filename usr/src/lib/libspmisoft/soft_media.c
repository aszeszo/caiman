...
#pragma ident	"@(#)soft_media.c	1.13	07/11/08 SMI"
...
static int		doumount(char *, char *, int, MediaType);
 ...
 *	in future calls to routines such as load_media().
 * Parameters:
...
/*
 * swi_eject_media()
 *	Eject the raw media device. Currently supports CDROM and floppy devices
 *      only.  Floppy drives are checked to make sure that the drive supports
 *      ejection.
 *
 ...
 *	device	-
 * Return:
 *      0 - Success (includes attempted ejection of floppies from non-
 *                   ejecting drives)
 *     -1 - Failure
 * Status:
 *	public
 */
int
swi_eject_media(char *device, MediaType mt)
{
	int	fd;
	...
	if (strncmp(device, "/dev/dsk/", 9) == 0)
		(void) snprintf(rawdevice, sizeof (rawdevice), "/dev/r%s",
		    strstr(device, "dsk/"));
	else if (strncmp(device, "/dev/rdsk/", 10) == 0)
		(void) snprintf(rawdevice, sizeof (rawdevice), "%s", device);
	...
	fd = open(rawdevice, O_RDONLY|O_NDELAY);
	if (fd < 0)
		return (-1);
	...
	case FLOPPY:
		{
			struct fd_drive fdstr;

			/* Can it be ejected? */
			if (ioctl(fd, FDGETDRIVECHAR, &fdstr) < 0) {
				ejectit = 0;
				rc = -1;
			} else {
				ejectit = fdstr.fdd_ejectable;
			}
		}
		break;
	}
...
duplicate_media(Module * current)
{
	...
	for (mp = current->sub; mp != NULL; mp = mp->next) {
		...
		if (mp->info.prod->p_zonename != NULL)
			mp2->info.prod->p_zonename =
			    xstrdup(mp->info.prod->p_zonename);
		if (mp->info.prod->p_inheritedDirs != NULL) {
			int nDirs;
			for (nDirs = 0;
			    mp->info.prod->p_inheritedDirs[nDirs];
			    nDirs++) {
				if (mp2->info.prod->p_inheritedDirs == NULL)
					mp2->info.prod->p_inheritedDirs =
					    (char **)xcalloc((nDirs + 1) *
					    sizeof (char *));
				else
					mp2->info.prod->p_inheritedDirs =
					    (char **)xrealloc(
				...
				mp2->info.prod->p_inheritedDirs[nDirs] =
				xstrdup(mp->info.prod->p_inheritedDirs[nDirs]);
			}
		}
		...
		walklist(mp->info.prod->p_clusters, dup_clstr_list,
		    (caddr_t)mp2);
		ap2 = NULL;
		for (ap = mp->info.prod->p_arches; ap != NULL;
		    ap = ap->a_next) {
			if (ap2 == NULL) {
		...
		/* Duplicate geos */
		tp2 = NULL;
		for (tp = mp->info.prod->p_geo; tp != NULL; tp = tp->next) {
			if (tp2 == NULL) {
				mp2->info.prod->p_geo = tp2 =
					(Module *)xcalloc(sizeof (Module));
				tp2->prev = NULL;
				tp2->head = tp2;
			} else {
			...
			tp2->type = tp->type;
			tp2->parent = mp2;
			tp2->info.geo = (Geo *)xcalloc(sizeof (Geo));
			tp2->info.geo->g_geo = xstrdup(tp->info.geo->g_geo);
			tp2->info.geo->g_name = xstrdup(tp->info.geo->g_name);
			tp2->info.geo->g_selected = tp->info.geo->g_selected;
			tp2->info.geo->g_locales =
					StringListDup(tp->info.geo->g_locales);
		}
	...
	if (current->info.media->med_zonename != NULL)
		new->info.media->med_zonename =
		    xstrdup(current->info.media->med_zonename);
...
dup_modinfo(Modinfo * mod, caddr_t data)
{
	...
	if (mod->m_loc_strlist != NULL)
		mp->m_loc_strlist = StringListDup(mod->m_loc_strlist);
	...
	if (mod->m_instances != NULL) {
		(void) dup_list(mod->m_instances, data);
	}
	/*
	...
	if (mod->m_next_patch != NULL) {
		(void) dup_list(mod->m_next_patch, data);
	}
...
/*ARGSUSED*/
static int
doumount(char *device, char *dir, int eject, MediaType mt)
{
	if (DirUmount(dir) < 0)
		return (ERR_UNMOUNT);

	if (eject)
		(void) swi_eject_media(device, mt);
...
read_product(Module * mod)
{
	...
	if (media->info.media->med_type == INSTALLED ||
		...
		if (snprintf(buf, sizeof (buf), "%s/%s", get_rootdir(),
			media->info.media->med_dir) >= sizeof (buf)) {
			return (ERR_UNMOUNT);
		}
		if (path_is_readable(buf) == FAILURE)
			return (ERR_UNMOUNT);
		if (snprintf(name, sizeof (name), "%s/.cdtoc", buf)
			>= sizeof (name)) {
			return (ERR_UNMOUNT);
		}
	} else {
		if (path_is_readable(media->info.media->med_dir) == FAILURE)
			return (ERR_UNMOUNT);
		if (snprintf(name, sizeof (name), "%s/.cdtoc",
			media->info.media->med_dir) >= sizeof (name)) {
			return (ERR_UNMOUNT);
		}
	}
	...
	while (fgets(buf, BUFSIZ, fp)) {
		if (buf[0] == '#' || buf[0] == '\n')
			...
			if ((cp = (char *)get_value(buf, '='))[0] == '/')
				...
				tp = xmalloc(strlen(media->info.media->med_dir)
				    + strlen(cp) + 2);
				(void) snprintf(tp,
				    strlen(media->info.media->med_dir)
				    + strlen(cp) + 2, "%s/%s",
...
canonical_path(char *dir, char *dir2)
{
	...
	if (dir[0] != '/') {
		if (getcwd(dirbuf, sizeof (dirbuf)) == NULL)
			return (NULL);
		if (strlcat(dirbuf, "/", sizeof (dirbuf)) >= sizeof (dirbuf))
			return (NULL);
		if (strlcat(dirbuf, dir, sizeof (dirbuf)) >= sizeof (dirbuf))
			return (NULL);
		cp = dirbuf;
	} else
		cp = dir;
	...
	if (realpath(cp, dir2) == NULL) {
		errno = 0;
		if (strlcpy(dir2, cp, MAXPATHLEN) >= MAXPATHLEN)
			return (NULL);
	}
	return (dir2);
}
...
load_patch_space_requirements(void)
{
	...
	while ((dp = readdir(dirp)) != (struct dirent *)0) {
		...
		(void) snprintf(path, MAXPATHLEN, "/tmp/%s", dp->d_name);
...
{
	char	s1[BUFSIZ + 1], pkgid[BUFSIZ + 1];
	ulong	d1, d2;
	...
	if (s1[0] != '\0') {
		if (strcmp(s1, "SPOOLED") != 0 && s1[0] != '/')
			return;
		pde = (struct patdir_entry *)xcalloc((size_t)
		    sizeof (struct patdir_entry));
...
dup_patchnum_list(struct patch_num *pnumlist)
{
	...
	for (; pnumlist != NULL; pnumlist = pnumlist->next) {
		pnum = (struct patch_num *)
		    xcalloc((size_t)sizeof (struct patch_num));
