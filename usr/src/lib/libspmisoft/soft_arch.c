...
#pragma ident	"@(#)soft_arch.c	1.5	07/11/12 SMI"
			...
			if (supports_arch(get_default_arch(), i->m_arch)) {
				...
				for (k = i; k != NULL; k = next_patch(k)) {
					if (a->a_selected &&
						 ...
						 * This is really ugly...
						 * See RFE# 4016940
						 * When doing a server upgrade,
						 * repartitioning is to take
						 * place, we may end up
						 * deselecting SUNWkvm pkgs for
						 * clients, soooooo....
						 */
						if (strncmp(k->m_pkgid,
						    "SUNWkvm", 7)) {
							k->m_status =
							    UNSELECTED;
						}
