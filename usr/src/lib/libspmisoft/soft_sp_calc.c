...
#pragma ident	"@(#)soft_sp_calc.c	1.9	07/11/09 SMI"
...
/*
 * reset current space table with no side effects
 * used when current space table must alternate
 */
void
set_global_space(FSspace **sp)
{
	cur_stab = sp;
}
...
/*
 * Add a product's space usage in blocks to the current spacetab.
 *	cdinf - source media information
 *	sp - file system space table or global space table if NULL
 *	p_rootdir - for non-global zone: if file system on separate
 *		slice, credit file system on that slice
 */
void
add_product(CD_Info *cdinf, FSspace **sp, char *p_rootdir)
{
	int i;
	FSspace **products_sp = NULL;
	...
	if (get_cd_size(cdinf) == 0)
		return;

	products_sp = load_def_spacetab(NULL);

	if (products_sp == NULL)
		return;

	reset_stab(products_sp);

	if (strcmp(p_rootdir, "/") == 0) {
		for (i = 0; products_sp[i]; i++) {
			fsp_add_to_field(products_sp[i],
			    FSP_CONTENTS_PRODUCTS,
			    get_cd_fs_size(cdinf,
			    map_fs_idx_from_mntpnt(
			    products_sp[i]->fsp_mntpnt)));
		}
	} else { /* non-global zone */

		/*
		 * Handle a non-global zone that contains a separate default
		 * file system defined in def_mnt_pnt[].
		 * i.e. /export/zone1/lu/a/usr where usr is a separate slice.
		 */

		for (i = 0; products_sp[i]; i++) {

			if (!z_path_is_inherited(products_sp[i]->fsp_mntpnt,
			    '\0', get_rootdir()) &&
			    !z_path_is_inherited(products_sp[i]->fsp_mntpnt,
			    '\0', "/"))
				fsp_add_to_field(products_sp[i],
				    FSP_CONTENTS_PRODUCTS,
				    get_cd_fs_size(cdinf,
	...
	add_spacetab(products_sp, sp, p_rootdir);

	free_space_tab(products_sp);
}
...
/*
 * Determine the bucket where the space will get credited and do it.
 * This function is used to read FS info from a non-global zone.
 * sp - file system
 * p_rootdir - zone root directory
 * mntpnt - mount point of NGZ
 * cb - counters of file system usage broken down
 * type_flags - any type flags set by subprocess in zone
 */
void
record_FS_info(FSspace **sp, char *p_rootdir, char *mntpnt, ContentsBrkdn *cb,
    int type_flags)
{
	register int	i;
	char tmnt[MAXPATHLEN]; /* temp buf for mntpnt */
	...
	/* Must be a full pathname. Determine which fsp_mntpnt bucket. */
	for (i = 0; sp[i] != NULL; i++) {
		if (sp[i]->fsp_flags & FS_IGNORE_ENTRY || mntpnt == NULL)
			continue;

		/* concatenate zone rootdir + current mountpoint */
		(void) strncpy(tmnt, p_rootdir, sizeof (tmnt));
		(void) strlcat(tmnt, sp[i]->fsp_mntpnt, sizeof (tmnt));

		/*
		 * if there is a mount point within a zone on its own slice
		 * eg, for zone named "zonex", its /usr is on a separate slice,
		 *	/export/zonex/lu/a/usr is a slice
		 */
		if ((!streq(mntpnt, "/") &&
		    streq(sp[i]->fsp_mntpnt, mntpnt)) ||
		    /* or if we've looked everywhere else */
		    streq(sp[i]->fsp_mntpnt, "/") ||
			/*
			 * or if the zone root is on a separate slice
			 * eg, /export is a slice and zone named "zonex" is on
			 * export, /export/zonex is prefixed with /export
			 */
		    strncmp(sp[i]->fsp_mntpnt, tmnt,
		    strlen(sp[i]->fsp_mntpnt)) == 0) {
			/* credit file system usage and synchronize breakdown */
			fsp_add_to_field(sp[i], FSP_CONTENTS_PKGD,
			    cb->contents_packaged);
			fsp_add_to_field(sp[i], FSP_CONTENTS_NONPKG,
			    cb->contents_nonpkg);
			fsp_add_to_field(sp[i], FSP_CONTENTS_PRODUCTS,
			    cb->contents_products);
			fsp_add_to_field(sp[i], FSP_CONTENTS_DEVFS,
			    cb->contents_devfs);
			fsp_add_to_field(sp[i], FSP_CONTENTS_SAVEDFILES,
			    cb->contents_savedfiles);
			fsp_add_to_field(sp[i], FSP_CONTENTS_PKG_OVHD,
			    cb->contents_pkg_ovhd);
			fsp_add_to_field(sp[i], FSP_CONTENTS_PATCH_OVHD,
			    cb->contents_patch_ovhd);
			sp[i]->fsp_cts.contents_inodes_used +=
			    cb->contents_inodes_used;
			sp[i]->fsp_flags |= type_flags;
			break;
		}
	}
}
 ...
 *		be added (if NULL, use the current global space table).
 *	p_rootdir - if non-NULL, the non-global zone mount point
 */
void
add_spacetab(FSspace **fstab, FSspace **sp, char *p_rootdir)
{
	int	i, j;
	char	catmnt[MAXPATHLEN];
	...
	for (i = 0; fstab[i] != NULL; i++) {
		...
		for (j = 0; sp[j] != NULL; j++) {
			if (sp[j]->fsp_flags & FS_IGNORE_ENTRY)
				continue;
			if (p_rootdir != NULL) {
				(void) strncpy(catmnt, p_rootdir,
				    sizeof (catmnt));
				(void) strlcat(catmnt, fstab[i]->fsp_mntpnt,
				    sizeof (catmnt));
			}
			if ((p_rootdir != NULL &&
			    is_an_intermediate_dir_of(
			    sp[j]->fsp_mntpnt, catmnt)) ||
			    is_an_intermediate_dir_of(
			    sp[j]->fsp_mntpnt, fstab[i]->fsp_mntpnt)) {
...
add_contents_brkdn(ContentsBrkdn *cr, FSspace *fs)
{
	...
	fsp_add_to_field(fs, FSP_CONTENTS_PRODUCTS, cr->contents_products);
