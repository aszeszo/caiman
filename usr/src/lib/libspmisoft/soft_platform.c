...
#pragma ident	"@(#)soft_platform.c	1.6	07/11/09 SMI"
...
load_platforms(Module * prod)
{
	...
	if (snprintf(buf, sizeof (buf), "%s/.platform",
		prod->info.prod->p_pkgdir) >= sizeof (buf)) {
		return (ERR_INVALID);
	}
	dirp = opendir(buf);
	if (dirp == (DIR *)0)
 ...
 */
void
load_installed_platforms(Module * prod, boolean_t zone)
{
	char		buf[MAXPATHLEN];
	PlatGroup	*pgp, **pgpp;
	Module		*mod;
	...
	/*
	 * Nonglobal zones don't have a .platform file,
	 * use the global zone's .platform information
	 */
	if (!zone) {
		if (snprintf(buf, sizeof (buf), "%s/var/sadm/system/admin",
			prod->info.prod->p_rootdir) >= sizeof (buf))
			return;
		...
		mod = get_localmedia();
		prod->info.prod->p_swcfg = mod->sub->info.prod->p_swcfg;
		prod->info.prod->p_platgrp = mod->sub->info.prod->p_platgrp;
		prod->info.prod->p_hwcfg = mod->sub->info.prod->p_hwcfg;
	}
}
...
swi_write_platform_file(char *rootdir, Module *prod)
{
	...
	(void) snprintf(path, sizeof (path),
	    "%s/var/sadm/system/admin/.platform", rootdir);
    ...
    Product *prod2)
{
	...
	for (pgp = merge_pltgrp; pgp; pgp = pgp->next) {
		...
		(void) snprintf(buf, sizeof (buf), "PLATFORM_GROUP=%s",
		    pgp->pltgrp_name);
		scriptwrite(fp, LEVEL1, generic, "LINE", buf, (char *)0);
		(void) snprintf(buf, sizeof (buf), "INST_ARCH=%s",
		    pgp->pltgrp_isa);
		scriptwrite(fp, LEVEL1, generic, "LINE", buf, (char *)0);

		/* write out individual platform entries */
		for (pp = pgp->pltgrp_members; pp; pp = pp->next) {
			(void) snprintf(buf, sizeof (buf), "PLATFORM_NAME=%s",
			    pp->plat_name);
			scriptwrite(fp, LEVEL1, generic,
			    "LINE", buf, (char *)0);
			if (pp->plat_uname_id) {
				(void) snprintf(buf, sizeof (buf),
				    "PLATFORM_ID=%s", pp->plat_uname_id);
				scriptwrite(fp, LEVEL1, generic, "LINE", buf,
				    (char *)0);
			}
			if (pp->plat_machine) {
				(void) snprintf(buf, sizeof (buf),
				    "MACHINE_TYPE=%s", pp->plat_machine);
				scriptwrite(fp, LEVEL1, generic, "LINE", buf,
				    (char *)0);
			}
			(void) snprintf(buf, sizeof (buf),
			    "IN_PLATFORM_GROUP=%s", pgp->pltgrp_name);
			scriptwrite(fp, LEVEL1, generic,
			    "LINE", buf, (char *)0);
		}
	}
...
read_platform_file(char *dir, char *filename, int is_core)
{
	...
	if (snprintf(path, sizeof (path), "%s/%s", dir, filename) >=
			sizeof (path)) {
		return (ERR_INVALID);
	}
...
process_error:
	...
	return ((status == 0) ? ERR_INVALID : status);
}
