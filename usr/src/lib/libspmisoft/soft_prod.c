...
#pragma ident	"@(#)soft_prod.c	1.32	07/11/09 SMI"
...
static void	load_local_package(Module *, char *);
...
static int	islocalpackage(char *, char *);
...
static void	mark_virtual_packages(Module *);
...
static char	metaclust_l10n_name[] = "SUNW_INSTALL_LIBSOFT";
...
swi_get_clustertoc_path(Module * mod)
{
	...
	/*
	 * If the locale is zh_TW.ISO8859-1 you'd first search
	 * Product/locale/zh_TW.ISO8859-1/.clustertoc then
	 * Product/locale/zh_TW/.clustertoc then
	 * Product/locale/zh/.clustertoc then
	 * Product/locale/C/.clustertoc
	 */
	system_locale = get_system_locale();
	if (system_locale) {
		locale = xstrdup(system_locale);
		if (snprintf(cluster_toc, sizeof (cluster_toc),
			"%s/locale/%s/%s",
			prod->info.prod->p_pkgdir,
			locale, CLUSTER_TOC_NAME) >= sizeof (cluster_toc)) {
				free(locale);
				return (NULL);
		}

		if (path_is_readable(cluster_toc) == SUCCESS) {
			free(locale);
			return (cluster_toc);
		}

		tmp_ptr = strchr(locale, '.');
		if (tmp_ptr) {
			*tmp_ptr = '\0';
			(void) snprintf(cluster_toc, sizeof (cluster_toc),
					"%s/locale/%s/%s",
					prod->info.prod->p_pkgdir,
					locale, CLUSTER_TOC_NAME);
			if (path_is_readable(cluster_toc) == SUCCESS) {
				free(locale);
				return (cluster_toc);
			}
	...
	/*
	 * Now try to read  Product/locale/C/.clustertoc
	 */
...
load_clusters(Module *prod, char *clustertoc)
{
	...
	/*
	 * If open of the clustertoc file fails and this is a non-global zone,
	 * try opening the file descriptor for the clustertoc file from the
	 * global zone before quitting.
	 */
	if (((fp = fopen(clustertoc, "r")) == NULL) &&
	    ((fp = get_fp_from_zone_fd(ZONE_FD_CLUSTERTOC)) == NULL)) {
	...
	while (fgets(buf, BUFSIZ, fp)) {
		...
		if (buf[0] == '#' || buf[0] == '\n' || strlen(buf) == 0)
			...
			str = get_value(buf, '=');
			if (ci_streq(str, "y") || ci_streq(str, "yes")) {
				info->m_status = REQUIRED;
			}
		} else if (strncmp(buf, "HIDDEN=", 7) == 0) {
			str = get_value(buf, '=');
			if (ci_streq(str, "y") || ci_streq(str, "yes")) {
				info->m_flags |= UI_HIDDEN;
			}
		} else if (strncmp(buf, "DEFAULT=", 8) == 0) {
			str = get_value(buf, '=');
			if (ci_streq(str, "y") || ci_streq(str, "yes")) {
				info->m_flags |= UI_DEFAULT;
			}
		} else if (strncmp(buf, "NAME=", 5) == 0)
			info->m_name = (char *)xstrdup(dgettext(
			    metaclust_l10n_name, get_value(buf, '=')));
			...
			write_message(LOGSCR, ERRMSG, LEVEL0,
				...
				"Invalid line in clustertoc: %s"), buf);
			return (ERR_BADENTRY);
		}
	}
	(void) fclose(fp);
...
load_pkginfo(Module *mod, char *name, int options)
{
	...
	if ((MediaType) options == INSTALLED ||
		...
		if (snprintf(dirname, sizeof (dirname), "%s/%s/%s",
			get_rootdir(),
			mod->info.prod->p_pkgdir, name) >= sizeof (dirname)) {
			return (ERR_INVALID);
		}
	} else {
		if (snprintf(dirname, sizeof (dirname), "%s/%s",
			mod->info.prod->p_pkgdir, name) >= sizeof (dirname)) {
			return (ERR_INVALID);
		}
	}
	...
	if (snprintf(infoname, sizeof (infoname), "%s/pkginfo",
				dirname) >= sizeof (infoname))
		return (ERR_INVALID);
	...
	while (fgets(buf, BUFSIZ, mp) != (char *)NULL) {
		if (strncmp(buf, "PKG=", 4) == 0) {
			...
			if (add_subset_locale_list(mod, info->m_loc_strlist) !=
			    SUCCESS) {
				(void) fclose(mp);
				free_modinfo(info);
				return (ERR_INVALID);
	 ...
	 */
	if ((MediaType)options != INSTALLED) {
		if (snprintf(infoname, sizeof (infoname), "%s/pkgmap", dirname)
				>= sizeof (infoname))
			return (ERR_INVALID);
		(void) calc_pkg_space(infoname, info, mod->info.prod);
	}
...
/*
 * reset_size_comp()
 *	Initializes values in a Size_comp structure to zero.
 * Parameters:
 *	sc	- Size_comp pointer
 * Return:
 *	none
 * Status:
 *	semi-private (internal library use only)
...
/*
 * get_packagetoc_path()
 * Kludge fix to support Star Office and Star Suite
 * Star Suite packages available only in Asian locales and Star Office
 * packages available in rest of the locale.
 * So if it is an asian locale, there will a different path for the
 * packagetoc. For others, the get the default packagetoc.
 *
 * Parameters:
 *	mod	-
 * Return:
...
get_packagetoc_path(Module * mod)
{
	...
	/*
	 * If the locale is zh_TW.ISO8859-1 you'd first search
	 * Product/locale/zh_TW.ISO8859-1/.packagetoc then
	 * Product/locale/zh_TW/.packagetoc then
	 * Product/locale/zh/.packagetoc then
	 * Product/locale/C/.packagetoc
	 */
	system_locale = get_system_locale();
	if (system_locale) {
		locale = xstrdup(system_locale);
		if (snprintf(package_toc, sizeof (package_toc),
			"%s/locale/%s/%s",
			prod->info.prod->p_pkgdir,
			locale, PACKAGE_TOC_NAME) >= sizeof (package_toc)) {
				free(locale);
				return (NULL);
		}

		if (path_is_readable(package_toc) == SUCCESS) {
			free(locale);
			return (package_toc);
		}
		...
		if (tmp_ptr) {
			*tmp_ptr = '\0';
			(void) snprintf(package_toc, sizeof (package_toc),
					"%s/locale/%s/%s",
					prod->info.prod->p_pkgdir,
					locale, PACKAGE_TOC_NAME);
			if (path_is_readable(package_toc) == SUCCESS) {
				free(locale);
				return (package_toc);
			}
	...
	locale = get_default_locale();
	if (snprintf(package_toc, sizeof (package_toc), "%s/locale/%s/%s",
			prod->info.prod->p_pkgdir, locale,
			PACKAGE_TOC_NAME) >= sizeof (package_toc))
		return (NULL);
	/*
	...
	if (path_is_readable(package_toc) == FAILURE) {
		(void) snprintf(package_toc, sizeof (package_toc), "%s/%s",
			prod->info.prod->p_pkgdir,
			PACKAGE_TOC_NAME);
	}
	return (package_toc);
...
get_orderfile_path(Module * mod)
{
	...
	if (snprintf(order_file, sizeof (order_file), "%s/%s",
			prod->info.prod->p_pkgdir,
			ORDER_FILE_NAME) >= sizeof (order_file)) {
		return (NULL);
	}
	return (order_file);
}
 ...
 *	mod	-
 * Return:
 *          SUCCESS -  If a .packagetoc file been loaded into the info struct
 *          !SUCCESS - If a .packagetoc file has not been loaded into the
 *                     info struct.
 * Status:
 *	private
 */
 ...
 *			   and .order files are to be used during the load
 * Return:
 *          SUCCESS - If a product Module has been loaded
 *          !SUCCESS - If a product Module has not been loaded
 * Status:
 *	private
 */
static int
load_product(Module *prod, int use_dotfiles)
{
	...
	if (pkgtoc != SUCCESS) {
		...
		while ((dp = readdir(dirp)) != (struct dirent *)0) {
			if (strcmp(dp->d_name, ".") == 0 ||
				...
				continue;
			loaded = SUCCESS;
			if (islocalpackage(prod->info.prod->p_pkgdir,
					    dp->d_name))
				load_local_package(prod, dp->d_name);
	...
	if (prod->info.prod->p_packages != NULL) {
		...
		/* look to see if the packages are local or not */
		mark_virtual_packages(prod);
...
{
	Modinfo	*info;
	Module	 *head, *current, *mod, *media_mod;
	Media *media;
	Node	   *np, *pkg_node, *cluster_node;
	...
	do {
		...
		if (strncmp(buf, "SUNW_CSRMEMBER", 14) == 0) {
			...
			pkg_node = findnode(prod->info.prod->p_packages,
							get_value(buf, '='));
			cluster_node = findnode(prod->info.prod->p_clusters,
						get_value(buf, '='));

			/*
			 * In the normal case, if there's a package listed in
			 * the clustertoc file that's not installed on the
			 * system being upgraded, we add it to the product tree
			 * as a null package so, among other things, we know
			 * not to add it on upgrade.
			 *
			 * In releases prior to Solaris 7, Starfires appeared
			 * as arch sun4u1. As of Solaris 7, they show up as
			 * sun4u.  This causes a problem for sun4u1-specific
			 * packages that had no sun4u-specific counterpart.
			 * Say we install a sun4u with Solaris 2.6 SUNWCXall.
			 * Said machine doesn't get either SUNWdrr or SUNWcvc,
			 * because the machine is a sun4u - not a sun4u1. The
			 * machine will, however be installed with a clustertoc
			 * that reports SUNWdrr and SUNWcvc as being part of
			 * the SUNWCXall metacluster. When that machine is
			 * upgraded to Solaris 7 and above, upgrade will notice
			 * that SUNWdrr and SUNWcvc were in the SUNWCXall
			 * metacluster on the system to be upgraded, but were
			 * not installed on that system. It'll therefore assume
			 * that the user elected not to install them, and
			 * therefore won't install them as part of the upgrade.
			 *
			 * This turns out to be an incorrect assumption.  They
			 * didn't install SUNWdrr and SUNWcvc because said
			 * packages weren't available for them to install in the
			 * first place.
			 *
			 * The fix is to treat all platform specific packages
			 * which are not in the system as new packages.
			 */

			if (pkg_node == NULL && cluster_node == NULL) {
				/*
				 * The package/cluster is not installed.
				 * Find out whether this is a platform
				 * specific package. If so remove from
				 * the list so that upgrade
				 * will treat this as a new package.
				 */

				/* looking for Media */
				media_mod = get_media_head();
				if (media_mod == NULL) {
				...
				media = NULL;
				if (media_mod->type == MEDIA)
					media = (Media *)media_mod->info.media;
				if (media == NULL ||
				    media->med_type == INSTALLED ||
				    media->med_type == INSTALLED_SVC) {
					if (fgets(buf, BUFSIZ, fp)) {
						buf[strlen(buf) - 1] = '\0';
					}
					continue;
				...
				np = findnode(media->med_deflt_prod->
			...
			if (pkg_node != NULL) {
				/*
				 * The package is installed by the
				 * previous install/upgrade
				 */
				info = (Modinfo *) pkg_node->data;
				current->info.mod = info;
				...
				/*
				 * The cluster is installed by the
				 * previous install/upgrade.
				 */
				mod = (Module *) cluster_node->data;
				dup_clstr_tree(mod, current);
				current->info.mod->m_flags |= PART_OF_CLUSTER;
			} else {
				/*
				 * The package/cluster is not installed.
				 * The platform specific package is handled
				 * already. So these packages should not
				 * be present after upgrade.
				 */
 ...
 */
static int
islocalpackage(char *dir, char *name)
{
	char	file[MAXPATHLEN];

	if (snprintf(file, sizeof (file), "%s/%s/pkginfo",
			dir, name) >= sizeof (file))
		return (0);

	if (access(file, R_OK) == 0) {
		(void) snprintf(file, sizeof (file), "%s/%s/pkgmap",
		    dir, name);
		if (access(file, R_OK) == 0)
			return (1);
	}
...
is4x(char *dir, char *name)
{
	...
	if (snprintf(info, sizeof (info), "%s/%s/_info", dir, name)
		>= sizeof (info)) {
		return (0);
	}
 ...
 */
static void
load_local_package(Module *prod, char *pkg_dir)
{
...
load_4x(Module *prod, char *name)
{
	...
	if (snprintf(dirname, sizeof (dirname), "%s/%s/_info",
		prod->info.prod->p_pkgdir, name) >= sizeof (dirname)) {
		return (ERR_NOPKG);
	}
	fp = fopen(dirname, "r");
	if (fp == (FILE *)NULL)
	...
	(void) snprintf(dirname, sizeof (dirname), "%s/%s",
	    prod->info.prod->p_pkgdir, name);
...
/*
 * mark_virtual_packages
 *	Find packages that are included in the .packagetoc, aren't present in
 *	the product tree, and are listed in the virtual package directory.
 *
 * Parameters:
 *	prod	- The product whose packages are to be marked
 * Return:
 *
 * Status:
 *	semi-private (internal library use only)
 */
static void
mark_virtual_packages(Module *prod)
{
	...
	for (np = np->next; np != packages->list; np = np->next) {
		...
		/* Look for the package on the image */
		if (snprintf(path, sizeof (path), "%s/%s",
			prod->info.prod->p_pkgdir,
			info->m_pkg_dir) >= sizeof (path)) {
			continue;
		}

		if (access(path, R_OK) != 0) {
			/*
			 * No package directory.  Make sure there's a
			 * file for it in the virtual package directory.
			 */
			...
			if (access(path, R_OK) == 0) {
				/* Found the file in the virtual directory. */
				info->m_flags |= IS_VIRTUAL_PKG;
			}
		}
	}
}
