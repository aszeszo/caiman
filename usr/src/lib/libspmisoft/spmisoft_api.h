...
/*
 * Type of dependency in the Depend structure
 */
typedef enum dependtype
{
	PREREQUISITE	= 0,
	INCOMPATIBLE	= 1,
	REVERSE		= 2,
...
/* Indicates a non default FS contains a non-global zone */
...
/* A geographic region - used to group locales */

typedef struct geo
{
	char		*g_geo;		/* the geo code */
	char		*g_name;	/* the full name of the geo */
	int		g_selected;	/* was this geo manually selected? */
	StringList	*g_locales;	/* The locales in this geo */
} Geo;
...
/* The maximum size of a single locale name */
#define	MAX_LOCALE	40

typedef struct size_comp {
	long	fs_size[N_LOCAL_FS];	/* filesystem sizes */
	char	*arch;			/* arch for this size component */
	StringList	*locales;	/* list of locales for this */
					/* size component */
} Size_comp;

typedef struct pd_size {
	struct size_comp	*info;	/* ptr to size component structure */
	struct pd_size	*next;		/* pointer to next size component */
} PD_Size;

typedef struct os_info {
	char *ospath;		/* /usr/lib/install/data/os/5.10 */
	char *osfile;		/* os.core.1 */
	struct module *prodToc;	/* pointer to prod_toc info */
} OS_Info;

typedef struct cd_info {
	char *cddir;		/* suppcd */
	char *cdname;		/* Solaris 10 Software Supplement CD */
	char *loccdname;	/* localized cd name to display */
	int  c_selected;	/* was this cd selected for installation? */
	boolean_t installer_wsr;	/* B_TRUE or B_FALSE */
	char *volid;		/* volume id of cd */
	char *installer;	/* installer cmd to launch */
	char *minirootopts;	/* miniroot options for launcher */
	int  cdsource;		/* source of cd (file system, kiosk, cd) */
	char *addpath;		/* YES or NO */
	struct module *prodToc;	/* pointer to prod_toc info */
} CD_Info;

typedef struct prod_toc {
	char *pdname;			/* pd file suffix of product on cd */
	char *locprodname;		/* localized product name to display */
	int  defInstall;		/* install by default */
	int  p_selected;		/* selected for installation */
	struct pd_file *pdfile;		/* pointer to prod_toc info */
} Product_Toc;

typedef struct pd_file {
	char *cd_name;			/* cd name */
	char *prodid;			/* prodid to pass to webstart sdk */
	char *prodname;			/* English product name */
	char *reqMeta;			/* required metacluster */
	int miniRoot;			/* miniroot install */
	char *compid;			/* compid to pass to webstart sdk */
	char *cluster;			/* cluster */
	StringList *itags;		/* ordered scripts.patches,packages */
	struct size_comp *gen_size;	/* generic size infomation (applies */
					/* to all architectures all locales) */
	struct pd_size *head_sizes;	/* head of size components list */
} PDFile;

typedef struct cdtoc {
	char *prodname;		/* PRODNAME */
	char *prodvers;		/* PRODVERS */
	char *proddir;		/* PRODDIR */
} CDToc;

typedef struct solcd_info {
	char *cdname;	/* Solaris 10 Languages(x86 Platform Edition */
	char *volid;	/* volume id of cd */
	char *installer;	/* installer cmd to launch */
	char *prodid;	/* prodid */
} SolCD_Info;

typedef  struct media_kit_info {
	char *osDir;		/* ex, /usr/lib/install/data/os/5.10 */
	char *osMediaDir;
	/* ex, /usr/lib/install/data/os/5.10/solaris10_global */
} Media_Kit_Info;
...
typedef struct depend
{
	...
	char		*d_pkgidb;	/* dependent package	*/
	char		*d_version;	/* which package version?	*/
	char		*d_arch;	/* which architecture?	*/
	char		*d_zname;	/* nonglobal zonename	*/
	DependType	d_type;		/* type of dependency	*/
...
typedef struct modinfo
{
	...
	char		 *m_expand_arch; /* Architecture supported by pkg */
	...
	char		 *m_l10n_pkglist;
					/*
					 * list of packages localized by this
					 * module; pkgid list separated by ','s
					 */
	L10N		 *m_l10n;	/* ptr to list of available L10Ns */
	...
	char		 *m_instdir;	/* current installation base dir */
	...
	boolean_t	 m_found_in_zone;  /* found a pkg in a zone to add */
	boolean_t m_pkgadd_processed;  /* state of pkgadd script generation */
	char		 *m_zonelist;  /* list of zones to install */
} Modinfo;
...
#define	UI_DEFAULT			0x0040
				/* whether this module is default	*/
#define	UI_HIDDEN			0x0080
				/* whether this module is hidden	*/
#define	UI_SHOW_CHLD			0x0100
				/* character install specific flag	*/
#define	UI_SUBMOD_L1			0x0200
				/* character install specific flag	*/
#define	UI_SUBMOD_L2			0x0400
				/* character install specific flag	*/
#define	UI_SUBMOD_L3			0x0800
				/* character install specific flag	*/
#define	UI_ADDED			0x1000
				/* character install specific flag	*/
#define	UI_REMVD			0x2000
				/* character install specific flag	*/
#define	IS_VIRTUAL_PKG			0x8000
				/*
				 * package is included in the
				 * .packagetoc for this image,
				 * but is not on this image.
				 */

#define	ZONE_SPOOLED			0x10000
				/*
				 * installed package has its
				 * zones spool area populated
				 * already
				 */
...
typedef struct product
{
	...
	ModStatus	 p_status;	/* said to be obsolete	*/
	char		*p_id;
	char		*p_pkgdir;
	char		*p_instdir;
	Arch		*p_arches;
		/* arch linked list - formerly global all_arches */
	...
	struct module	*p_geo;
	struct module	*p_cd_info;
	struct module	*p_os_info;
	Node		*p_orphan_patch;
	char		*p_rootdir;
	...
	char		*p_zonename;	/* non-global zonename or NULL */
	char		**p_inheritedDirs;
} Product;
...
typedef struct media {
    ...
    char		*med_zonename;	/* non-global zonename or NULL */
} Media;
...
typedef struct module
{
	...
	union {
		...
		Geo	*geo;	/* actual geo information		*/
		Category *cat;  /* actual locale information		*/
		CD_Info	*cdinf;	/* actual cd information		*/
		Product_Toc *pdinf;	/* actual pd file information	*/
		OS_Info *osinf;	/* actual os file information	*/
	} info;
...
typedef struct view
{
	...
	union {
		...
		Geo	*v_geo;		/* actual geo information */
...
#define	CLSSIZ  64	/* Borrowed from /usr/include/pkgstrct.h */
struct filediff {
...
typedef struct fsspace {
	...
	int	fsp_reqd_free_percent;	/* application-provided required */
					/* free-space percentage. */
	int	fsp_reqd_free_kb;	/* amount of extra free space */
					/*  required. */
...
typedef enum valstage {
	...
	VAL_ANALYZE_END,
	VAL_UPG_BEGIN,
	VAL_EXEC_LOCAL_PKGADD,
	VAL_EXEC_VIRTUAL_PKGADD,
	VAL_EXEC_PKGRM,
	VAL_EXEC_REMOVEF,
	VAL_EXEC_LOCAL_SPOOL,
	VAL_EXEC_VIRTUAL_SPOOL,
	VAL_EXEC_RMTEMPLATE,
	VAL_EXEC_RMDIR,
	VAL_EXEC_RMSVC,
	VAL_EXEC_RMPATCH,
...
#define	WILDCARD_METACLUSTER	"ALL"
...
#define	CDTOC_NAME  		".cdtoc"
#define	VOLINF_NAME  		".volume.inf"
...
boolean_t check_sw_depends(void);
Depend * get_depend_pkgs(void);

/* soft_choosemedia.c */

void	setAutoEject(int);
char	*getCDdevice();
void	eject_disc(char *);
int	mount_disc(char *, char *);
int	verify_solaris_image(char *, StringList **, StringList **);
int	isAutoEject();
int	have_disc_in_drive(char *);
int	umount_dir(char *);
int	mount_path(char *, char *);
...
int	file_will_be_upgraded_db(char *, char *, int);
void	end_upgraded_file_scan(void);

/* install.c */

Module *load_installed(char *, boolean_t);
Modinfo *next_patch(Modinfo *);
Modinfo *next_inst(Modinfo *);

/* locale.c */
Module	*get_all_locales(void);
int	select_locale(Module *, char *, int);
int	deselect_locale(Module *, char *);
int	valid_locale(Module *, char *);
int	locale_list_selected(StringList **);
char	*get_init_default_system_locale(void);
char	*get_default_system_locale(void);
int	set_default_system_locale(char *);
char	*get_system_locale(void);
char	*get_locale_geo(char *);
int	save_locale(char *, char *);
int	get_sys_locale_list(char *, char ***);
void	build_locale_list(void);
char	*get_composite_locale(char *);

/* soft_geo.c */
Module	*get_all_geos(void);
int	valid_geo(Module *, char *);
int	select_geo(Module *, char *);
int	deselect_geo(Module *, char *);
void	generate_locgeo_lists(char ***, char ***);
char	*geo_name_from_code(char *);

/* soft_launcher.c */
void	parsePackagesToBeAdded();
void	create_dispatch_table();
void	setup_launcher(int);
...
int	eject_media(char *, MediaType);   /* CD-ROM or floppy only */
void	set_eject_on_exit(int);
Module	*get_media_head(void);
Module	*find_media(char *, char *);

/* soft_prodsel.c */
Module	*add_cd_module(Module *, char *, char *, char *);
Module	*add_os_module(Module *, char *, char *);
Module	*add_comp_module(Module *, char *, char *, int);
Module	*get_all_cds();
int	select_cd(Module *, char *);
int	deselect_cd(Module *, char *);
int	select_component(Module *, char *);
int	deselect_component(Module *, char *);
long	get_cd_fs_size(CD_Info *, FileSys);
long	get_cd_size(CD_Info *);
long	get_component_fs_size(Product_Toc *, FileSys);
long	get_component_size(Product_Toc *);
void	add_to_separate_zone_FSs(char *);
boolean_t separate_zone_FS(char *);


/* soft_update_action.c */
Module		*get_localmedia(void);
int		upgrading_clients();
void		set_upgrade_clients(int upg);
...
int	map_fs_idx_from_mntpnt(char *);
int	map_zone_fs_idx_from_mntpnt(char *, char *);
int	is_upgrade(void);
int	writeStringListToFile(char *, StringList *);
StringList	*readStringListFromFile(char *);
void	run_parse_dynamic_clustertoc();
int	get_useAltImage();
void	set_useAltImage(int);
char 	*get_device();
char 	*get_rawdevice();
...
int	upg_select_geo(Module *, char *);
int	upg_deselect_geo(Module *, char *);
int	upg_select_locale(Module *, char *);
int	upg_deselect_locale(Module *, char *);
...
FSspace **calc_package_space(Module *, ModStatus);
...
int	readProductTables();
void	readCDInfo(CD_Info *);
void	readProductToc(CD_Info *);
char	*get_loc_cdname(char *);
char	*get_loc_cdhelp(char *);
char	*get_loc_prodhelp(char *);
void	parsePDfile(CD_Info *);
void	setMkit(Media_Kit_Info *);
Media_Kit_Info	*getMkit();

/* soft_webstart_util.c */
char	*readInText(char *);
int	writeOutText(char *, char *, char *);
int	concatFiles(StringList *, char *);
int	copyFile(char *, char *, boolean_t);
int	copyDir(char *, char *);
int	mkdirs(char *);
int	pingHost(char *);
CDToc	*readCDTOC(char *);
void	free_cdtoc(CDToc *);
void	setWebstartLocale(char *);
char	*getWebstartLocale();
char	*getLocString();
void	check_boot_environment();
int	isBootFromDisc();
int	installAfterReboot();
