...
#pragma ident	"@(#)soft_version.c	1.4	07/11/12 SMI"
...
pkg_tokanize(char * toks[], char buf[])
{
	...
	/*
	 * Just look at the beginning version string pf PKG_SUN_VER_TOK;
	 * the rest is ignored.  For example, "4.1-FCS" would be read as
	 * "4.1"
	 */
	cp = toks[PKG_SUN_VER_TOK];
	while (*cp) {
	    if (!isdigit((unsigned) *cp) && (*cp != '.')) {
		*cp = '\0';
		break;
	    }
	    cp++;
