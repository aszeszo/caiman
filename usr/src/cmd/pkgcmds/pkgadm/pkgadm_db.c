...
#pragma ident	"@(#)pkgadm_db.c	1.4	06/02/27 SMI"
...
convert_to_db(int argc, char **argv)
{
	...
	if (pcroot[strlen(pcroot) - 1] == '/') {
		...
		if (snprintf(contents, PATH_MAX, "%svar/sadm/install/contents",
		    pcroot) >= PATH_MAX ||
		    snprintf(contents_save, PATH_MAX,
			"%svar/sadm/install/contents.save",
			pcroot) >= PATH_MAX) {
			result = 1;
			goto to_db_done;
		}

		if (snprintf(path, PATH_MAX, "%svar/sadm/pkg", pcroot)
		    >= PATH_MAX) {
			log_msg(LOG_MSG_ERR,
		...
		if (snprintf(contents, PATH_MAX, "%s/var/sadm/install/contents",
		    pcroot) >= PATH_MAX ||
		    snprintf(contents_save, PATH_MAX,
			"%s/var/sadm/install/contents.save",
			pcroot) >= PATH_MAX) {
			result = 1;
			goto to_db_done;
		}

		if (snprintf(path, PATH_MAX, "%s/var/sadm/pkg", pcroot)
		    >= PATH_MAX) {
			log_msg(LOG_MSG_ERR,
			    gettext(MSG_FILE_NAME_TOO_LONG), pcroot);
	...
	if (result == 0 && global_get_contents == 0) {
		...
		if (pcroot[strlen(pcroot) - 1] == '/') {
			...
			e = snprintf(contents_save, PATH_MAX,
			    "%svar/sadm/install/contents.save", pcroot);

		} else {

			e = snprintf(contents_save, PATH_MAX,
			    "%s/var/sadm/install/contents.save", pcroot);
