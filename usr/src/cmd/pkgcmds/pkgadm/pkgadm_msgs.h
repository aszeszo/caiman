...
#if 0
/* This usage is correct if the SQL database is supported */
...
"\n" \
"pkgadm upgrade [-R rootpath]\n" \
"\n" \
"\t- Upgrade to new improved install database support.\n" \
"\n" \
"pkgadm revert [-R rootpath]\n" \
"\n" \
"\t- Revert from improved install database support to use of the text \n" \
"\t  install database in use since Solaris 2.0.\n" \
"\n" \
"pkgadm refresh [-R rootpath]\n" \
"\n" \
"\t- Refresh text database cache for those those applications which do \n" \
"\t  not yet use the improved install database.\n" \
"\n" \
"pkgadm dbstatus [-R rootpath]\n" \
"\n" \
"\t- Returns 'text' if the text install database in use since Solaris 2.0\n" \
"\t  is the current install database in use.  Returns 'db' if the improved\n" \
"\t  database is in use.\n" \
"\n" \
"pkgadm -V\n" \
...
#else
/* This usage is corect if the SQL database is NOT supported */
#define	MSG_USAGE		gettext(\
"usage:\n" \
"\n" \
"pkgadm addcert  [-ty] [-a app] [-k keystore] [-e keyfile]\n" \
...
#define	DB_EXISTS_ADMONISH	gettext(\
	"\nInvalid response '%s'.  " \
	"Please answer ('y' or 'n', default is 'n')\n")

#define	DB_EXISTS_WARN	gettext(\
	"There is an existing install database.  If you perform an upgrade " \
	"operation,\nthe currently registered install data will be replaced " \
	"entirely by data\nconverted from install data text files.  " \
	"Normally when there is an existing\ninstall database, you should " \
	"not perform the upgrade operation.\n\n")
...
#define	DB_NONEXISTENCE	gettext( \
...
#define	MSG_DB_OP_FAILED	gettext(\
	"Unexpected error accessing the database")

#define	MSG_NO_DROP_DB		gettext(\
	"Unable to remove the existing database prior to upgrade "\
	"from text data.")

#define	MSG_DB_OP_FAILSTR	gettext(\
	"Error accessing the database: %s")
...
#define	DB_EXISTS_PROMPT	gettext(\
