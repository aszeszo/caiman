...
#pragma ident	"@(#)pkgadm_contents.c	1.4	06/02/27 SMI"
...
convert_db_to_contents(genericdb *pdb, const char *pcroot, int refresh)
{
	...
	if (pcroot[strlen(pcroot) -1] == '/') {
		if (snprintf(path, PATH_MAX, "%svar/sadm/install/contents",
		    pcroot) >= PATH_MAX) {
			log_msg(LOG_MSG_ERR,
		...
		if (snprintf(path, PATH_MAX, "%s/var/sadm/install/contents",
		    pcroot) >= PATH_MAX) {
			log_msg(LOG_MSG_ERR,
			    gettext(MSG_FILE_NAME_TOO_LONG), pcroot);
			result = 1;
