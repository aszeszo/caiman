...
class DiskWindow(InnerWindow):
    ...
    STATIC_GPT_PARTITION_HEADERS = [
        (15, _("Partition Type"), _("Partition Type")),
        (10, _(" Size(GB)"), _(" Size(GB)"))
    ]

    EDIT_GPT_PARTITION_HEADERS = [
        (15, _("Partition Type"), _("Partition Type")),
        (10, _(" Size(GB)"), _(" Size(GB)")),
        (7, _(" Avail"), _(" Avail"))
    ]
    ...
    def ui_obj(self, part):
        ...
        if isinstance(part, Disk):
            self._ui_obj = UIDisk(self.tc, part, parent=None)
    ...
    def set_disk_info(self, ui_obj=None, disk_info=None, no_part_ok=False):
        ...
        if hasattr(disk_info, "label") and disk_info.label == "GPT":
            self.has_gpt_data = True
            self.has_partition_data = False
        else:
            self.has_gpt_data = False
        ...
        if self.has_gpt_data:
            if self.editable:
                self.headers = DiskWindow.EDIT_GPT_PARTITION_HEADERS
                ...
                self.headers = DiskWindow.STATIC_GPT_PARTITION_HEADERS

        elif self.has_partition_data:
            if self.editable:
        ...
        if self.has_gpt_data:
            self.ui_obj.add_unused_gpt_parts()
        else:
    ...
    def print_data(self):
        ...
        GPT Partitions - fill the left side, then remaining partitions on the
        right side.
        '''

        parts_in_use = self.ui_obj.get_parts_in_use()

        if self.has_partition_data or self.has_gpt_data:
            max_parts = MAX_PRIMARY_PARTS
        else:
            max_parts = min(len(parts_in_use), self.left_win.area.lines)

        win = self.left_win
        y_loc = 0
        for part_index, data in enumerate(parts_in_use):
            data_size = data.size.get(Size.gb_units)
            ...
            if y_loc >= max_parts:
                if win is self.left_win:
                    ...
                    num_extra = len(parts_in_use) - part_index
                    if self.has_partition_data or self.has_gpt_data:
            ...
            field = 0
            win.add_text(str(data.get_description()), y_loc, x_loc,
                         self.headers[field][0] - 1)
            x_loc += self.headers[field][0]
            field += 1
            if not self.has_partition_data and not self.has_gpt_data:
                win.add_text(str(data.name), y_loc, x_loc,
                             self.headers[field][0] - 1)
                x_loc += self.headers[field][0]
                field += 1

            win.add_text(locale.format("%*.1f", (self.headers[field][0] - 1,
                data_size)), y_loc, x_loc, self.headers[field][0] - 1)
    ...
    def _update_avail_space(self, part_number=None, part_info=None):
        ...
        max_space = locale.format("%*.1f",
            (self.headers[field][0], max_space.get(Size.gb_units)))
    ...
    def change_type(self, dummy):
        '''Cycle the type for the currently active object, and update its field
        '''
        ...
        self.set_disk_info(disk_info=part_info.parent.doc_obj, no_part_ok=True)
        num_parts = len(self.ui_obj.get_parts_in_use())
        # If the partition UI object no longer exists, it can't be activated
        # so activate its closest neighbour. This only normally happens if a
        # GPT partition, beyond the number that Solaris supports, was deleted.
        if part_order >= num_parts:
            part_order = num_parts - 1
...
def resize_validate(edit_field, disk_win=None):
    ...
    if new_size_byte > max_size_byte:
        ...
        LOGGER.debug("Requested partition resize exceeds maximum size: "
                     "Clamping %d bytes to %d bytes",
                     new_size_byte, max_size_byte)
        new_size = max_size
...
def on_exit_edit(edit_field, disk_win):
    '''On exit, clean up user edits and normalize the partition size
       visual representation. eg -> "7" or "7." -> "7.0"
    '''

    LOGGER.debug("on_exit_edit()")
    # The partition has already been resized so use it as the definitive
    # reference of what should be displayed in the edit field.
    real_size = edit_field.data_obj.size
    real_size_rounded = round(real_size.get(Size.gb_units), 1)
    text = locale.format("%.1f", real_size_rounded)
    LOGGER.debug("Setting edit field text value: %s", text)
    edit_field.set_text(text)
...
def get_minimum_size(target_controller):
    '''Returns the minimum disk space needed for installation, as a Size object
    '''
