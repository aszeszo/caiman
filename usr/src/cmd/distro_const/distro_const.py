...
from osol_install.distro_const.DC_checkpoint import DC_add_finalizer_scripts, \
    step_from_name, DC_determine_resume_step, DC_verify_resume_step, \
    DC_delete_checkpoint, DC_verify_resume_state, shell_cmd, checkpoints, \
    DC_checkpoint_setup, DC_execute_checkpoint, DC_verify_manifest_filename 
from osol_install.distro_const.dc_ti import dc_create_zfs_build_data_area, \
    dc_create_ufs_build_data_area, dc_create_build_area 
from osol_install.distro_const.DC_tm import DC_populate_pkg_image 
from osol_install.distro_const.dc_utils import get_manifest_value, \
    setup_dc_logfile_names, add_file_logging, cleanup_dir, \
    get_manifest_boolean, setup_dc_logging 
from osol_install.distro_const.DC_defs import DC_LOGGER_NAME, \
    DC_MANIFEST_DATA, IMAGE_INFO_FILE, IMAGE_INFO_IMAGE_SIZE_KEYWORD, \
    CHECKPOINT_RESUME, BUILD_DATA, DISTRO_NAME, PKG_IMAGE, LOGS, \
    TMP, STOP_ON_ERR, MEDIA, SUCCESS, BOOT_ARCHIVE
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
def dc_create_image_info(mntpt):
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    """
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    stepname = get_manifest_value(manifest_server_obj, CHECKPOINT_RESUME)
    ...
    if subcommand == "build":
        ...
        if do_list == True and (pause == True or resume == True
                             or step_resume == True):
        ...
        if do_list == True:
            ...
            for step_obj in cp.step_list:
                if not laststep == -1 \
                    ...
                    r_flag = " "	
                dc_log.error("%s%s%s" % (step_obj.get_step_name().ljust(15),
                                         r_flag.center(10),
    ...
    if (dataset != None):
        ...
        if (have_empty_snapshot(cp)):
            ...
            try:
                ...
                ret = -1
            if ret == -1:
                dc_log.error("Error destroying the build_data area")
                return -1

            ret = dc_create_zfs_build_data_area(cp)
            if ret == -1:
                dc_log.error("Error creating the ZFS build_data area")
                return -1
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    ...
    # logfiles (logs) and boot archives (boot_archive).
    ...
    if status == 0:
        ...
        dc_create_image_info(cp.get_build_area_mntpt() + PKG_IMAGE) 
    elif status == -1:
        cleanup_dir(cp.get_build_area_mntpt() + TMP)
        dc_log.info("Build completed " + time.asctime(time.localtime()))
        return 0 

    stop_on_err_bool = get_manifest_boolean(manifest_server_obj, STOP_ON_ERR)
    ...
    finalizer_obj = DCFinalizer([manifest_server_obj.get_sockname(),
                                 pkg_img_area, tmp_area,
                                 ba_build_area, media_dir])
    ...
    status = DC_add_finalizer_scripts(cp, manifest_server_obj, finalizer_obj)
    if (status != SUCCESS):
        dc_log.info("Build completed " + time.asctime(time.localtime()))
        dc_log.info("Build failed.")
        return (status)
...
if __name__ == "__main__":
    ...
    ret_value = 1 # this will be set to 0 if main_func() succeeds
