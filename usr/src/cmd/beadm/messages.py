...
class Msgs:
    ...
    mBeadmErr[BEADM_ERR_BE_EXISTS] = "BE %s already exists. Please choose " + \
        "a different BE name."
    mBeadmErr[BEADM_ERR_BE_DOES_NOT_EXIST] = "%s does not exist or appear " + \
        "to be a valid BE.\nPlease check that the name of the BE provided " + \
        " is correct."
    mBeadmErr[BEADM_ERR_NO_BES_EXIST] = "No boot environments found on " + \
        "this system."
    mBeadmErr[BEADM_ERR_CREATE] = "Unable to create %(0)s.\n%(1)s"
    mBeadmErr[BEADM_ERR_DESTROY] = "Unable to destroy %(0)s.\n%(1)s"
    mBeadmErr[BEADM_ERR_DESTROY_ACTIVE] = "%(0)s is the currently active " + \
        "BE and cannot be destroyed.\nYou must boot from another BE in " + \
        "order to destroy %(1)s."
    mBeadmErr[BEADM_ERR_MSG_SUB] = "Fatal error. No message associated " + \
        "with index %d"
    mBeadmErr[BEADM_ERR_ILL_SUBCOMMAND] = "Illegal subcommand %s"
    mBeadmErr[BEADM_ERR_INVALID_RESPONSE] = "Invalid response. Please " + \
        "enter 'y' or 'n'."
    ...
    mBeadmErr[BEADM_ERR_MOUNT_EXISTS] = "%s is already mounted.\nPlease " + \
        "unmount the BE before mounting it again."
    mBeadmErr[BEADM_ERR_MOUNTED] = "Unable to destroy %(0)s.\nIt is " + \
        "currently mounted and must be unmounted before it can be " + \
        "destroyed.\nUse 'beadm unmount %(1)s' to unmount the BE before " + \
        "destroying\nit or 'beadm destroy -fF %(2)s'."
    mBeadmErr[BEADM_ERR_MOUNTPOINT] = "Invalid mount point %s. Mount " + \
        "point must start with a /."
    mBeadmErr[BEADM_ERR_MUTUALLY_EXCL] = "Invalid options: %s are " + \
        "mutually exclusive."
    ...
    mBeadmErr[BEADM_ERR_NOT_SUPPORTED_NGZ] = "beadm is not supported in " + \
          "a non-global zone."
    ...
    mBeadmErr[BEADM_ERR_SHARED_FS] = "%s is a shared file system and it " + \
        "cannot be unmounted."
    mBeadmErr[BEADM_ERR_SNAP_DOES_NOT_EXISTS] = "%s does not exist or " + \
        "appear to be a valid snapshot.\nPlease check that the name of " + \
    ...
    mBeadmErr[BEADM_ERR_UNMOUNT_ACTIVE] = "%s is the currently active " + \
        "BE.\nIt cannot be unmounted unless another BE is the currently " + \
        "active BE."
    mBeadmErr[BE_ERR_ZONES_UNMOUNT] = "Unable to destroy one of %(0)s's " + \
        "zone BE's.\nUse 'beadm destroy -fF %(1)s' or 'zfs -f destroy " + \
        "<dataset>'."
    mBeadmErr[BEADM_ERR_PERMISSIONS] = "You have insufficient privileges " + \
        "to execute this command.\nEither use 'pfexec' to execute the " + \
        "command or become superuser."
    ...
    mBeadmOut[BEADM_MSG_ACTIVE_ON_BOOT] = "The BE that was just " + \
        "destroyed was the 'active on boot' BE.\n%s is now the 'active " + \
        "on boot' BE. Use 'beadm activate' to change it.\n"
    mBeadmOut[BEADM_MSG_DESTROY] = "Are you sure you want to destroy %s? " + \
